...
namespace Pathfinding
{
    ...
    public class GraphUpdateSceneEditor : EditorBase
    {
        ...
        protected override void Inspector()
        {
            ...
            if (!points.hasMultipleDifferentValues && points.arraySize == 0)
            {
                if (scripts[0].GetComponent<PolygonCollider2D>() != null)
                {
                    ...
                    scripts[0].GetComponent<Collider>() != null
                    || scripts[0].GetComponent<Collider2D>() != null
                )
                {
            ...
            if (scripts.Length == 1 && scripts[0].points.Length >= 3)
            {
                ...
                if (
                    Mathf.Min(Mathf.Min(Mathf.Abs(size.x), Mathf.Abs(size.y)), Mathf.Abs(size.z))
                    < 0.05f
                )
                {
                    EditorGUILayout.HelpBox(
        ...
        void DrawPenaltyField()
        {
            ...
            if (
                !FindProperty("penaltyDelta").hasMultipleDifferentValues
                && FindProperty("penaltyDelta").intValue < 0
            )
            {
                EditorGUILayout.HelpBox(
        ...
        void DrawTagField()
        {
            if (PropertyField("modifyTag"))
            {
                ...
                EditorGUI.BeginChangeCheck();
                var newTag = EditorGUILayoutx.TagField(
                    "Tag Value",
                    tagValue.intValue,
        ...
        public void OnSceneGUI()
        {
            ...
            if (Tools.current != Tool.View && Event.current.type == EventType.Layout)
            {
                for (int i = 0; i < script.points.Length; i++)
                {
                    HandleUtility.AddControl(
                        -i - 1,
                        HandleUtility.DistanceToLine(points[i], points[i])
                    );
                }
            }
            ...
            if (Event.current.shift && Tools.current == Tool.Move)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < points.Count; i++)
                    {
                        float dist = HandleUtility.DistanceToLine(
                            points[i],
                    ...
                    if (didHit)
                    {
                        ...
                        else if (points.Count > 0)
                        {
                            ...
                            Handles.DrawDottedLine(
                                points[(insertionIndex - 1 + points.Count) % points.Count],
                                rayhit,
                                8
                            );
                            Handles.DrawDottedLine(
                                points[insertionIndex % points.Count],
                                rayhit,
                                8
                            );
                            SphereCap(
