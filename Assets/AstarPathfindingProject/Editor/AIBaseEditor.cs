    ...
    public class BaseAIEditor : EditorBase
    {
        ...
        protected void AutoRepathInspector()
        {
            ...
            if (!mode.hasMultipleDifferentValues)
            {
                ...
                else if (modeValue == AutoRepathPolicy.Mode.Dynamic)
                {
                    ...
                    EditorGUILayout.HelpBox(
                        "The path will be recalculated at least every "
                            + maxInterval.floatValue.ToString("0.0")
                            + " seconds, but more often if the destination changes a lot",
                        MessageType.None
                    );
                }
                EditorGUI.indentLevel--;
        ...
        protected void DebugInspector()
        {
            ...
            if (debug)
            {
                ...
                EditorGUILayout.LabelField(
                    "Remaining distance",
                    ai.remainingDistance.ToString("0.00")
                );
        ...
        protected override void Inspector()
        {
            ...
            if (isAIPath)
            {
                ...
                int acc = acceleration.hasMultipleDifferentValues
                    ? -1
                    : (acceleration.floatValue >= 0 ? 1 : 0);
                var nacc = EditorGUILayout.Popup(
                    "Max Acceleration",
                    acc,
                    new[] { "Default", "Custom" }
                );
                ...
                Popup(
                    ...
                    new[]
                    {
                        new GUIContent("ZAxisForward (for 3D games)"),
                        new GUIContent("YAxisForward (for 2D games)")
                    }
                );
            }
            else
            ...
            if (canUseGravity)
            {
                ...
                var ngrav = EditorGUILayout.Popup(
                    "Gravity",
                    grav,
                    new[] { "None", "Use Project Settings", "Custom" }
                );
                if (EditorGUI.EndChangeCheck())
                {
                    if (ngrav == 0)
                        ...
                        float.IsNaN(gravity.vector3Value.x) || gravity.vector3Value == Vector3.zero
                    )
            ...
            else
            {
                ...
                EditorGUILayout.Popup(
                    new GUIContent(
                        gravity.displayName,
                        "Disabled because a non-kinematic rigidbody is attached"
                    ),
                    0,
