    ...
    public class GridGraphEditor : GraphEditor
    {
        ...
        public override void OnInspectorGUI(NavGraph target)
        {
            ...
            if (graph.collision.use2D)
            {
                if (
                    Mathf.Abs(
                        Vector3.Dot(Vector3.forward, Quaternion.Euler(graph.rotation) * Vector3.up)
                    ) < 0.9f
                )
                {
        ...
        {
            if (graph.aspectRatio != 1)
                return true;
            if (IsHexagonal(graph))
                return false;
        ...
        InspectorGridMode DetermineGridType(GridGraph graph)
        {
            ...
            if (adv || (hex && iso))
                return InspectorGridMode.Advanced;
            if (hex)
                return InspectorGridMode.Hexagonal;
            if (iso)
                return InspectorGridMode.IsometricGrid;
        ...
        void DrawInspectorMode(GridGraph graph)
        {
            ...
            var newMode = (InspectorGridMode)
                EditorGUILayout.EnumPopup("Shape", (System.Enum)graph.inspectorGridMode);
        ...
        GUIContent[] hexagonSizeContents =
        {
            ...
            new GUIContent(
                "Hexagon Diameter",
                "Distance between two opposing vertices on the hexagon"
            ),
            new GUIContent(
                "Node Size",
        ...
        void DrawFirstSection(GridGraph graph)
        {
            ...
            if (graph.inspectorGridMode == InspectorGridMode.Hexagonal)
            {
                ...
                graph.inspectorHexagonSizeMode = (InspectorGridHexagonNodeSize)
                    EditorGUILayout.EnumPopup(
                        new GUIContent("Hexagon Dimension"),
                        graph.inspectorHexagonSizeMode
                    );
                float hexagonSize = GridGraph.ConvertNodeSizeToHexagonSize(
                    graph.inspectorHexagonSizeMode,
                    graph.nodeSize
                );
                hexagonSize = (float)System.Math.Round(hexagonSize, 5);
                newNodeSize = GridGraph.ConvertHexagonSizeToNodeSize(
                    graph.inspectorHexagonSizeMode,
                    EditorGUILayout.FloatField(
                        hexagonSizeContents[(int)graph.inspectorHexagonSizeMode],
                        hexagonSize
                    )
                );
            ...
            if (
                graph.inspectorGridMode == InspectorGridMode.IsometricGrid
            ...
            )
            {
                graph.aspectRatio = EditorGUILayout.FloatField(
                    new GUIContent(
                        "Aspect Ratio",
                        "Scaling of the nodes width/depth ratio. Good for isometric games"
                    ),
                    graph.aspectRatio
                );
            ...
            if (
                (nodeSizeChanged && locked)
                || (newWidth != graph.width || newDepth != graph.depth)
        ...
        void DrawRotationField(GridGraph graph)
        {
            if (graph.is2D)
            {
                ...
                if (angle < 0)
                    angle += 360;
                if (Mathf.Abs(angle - Mathf.Round(angle)) < 0.001f)
            ...
            {
                graph.rotation = RoundVector3(
                    EditorGUILayout.Vector3Field("Rotation", graph.rotation)
                );
            }
        }
        ...
        {
            lockStyle =
                lockStyle ?? AstarPathEditor.astarSkin.FindStyle("GridSizeLock") ?? new GUIStyle();

            GUILayout.BeginHorizontal();
            GUILayout.BeginVertical();
            newWidth = EditorGUILayout.IntField(
                new GUIContent("Width (nodes)", "Width of the graph in nodes"),
                graph.width
            );
            newDepth = EditorGUILayout.IntField(
                new GUIContent(
                    "Depth (nodes)",
                    "Depth (or height you might also call it) of the graph in nodes"
                ),
                graph.depth
            );
            ...
            locked = GUI.Toggle(
                ...
                new GUIContent(
                    "",
                    "If the width and depth values are locked, "
        ...
        {
            var isometricGUIContent = new GUIContent(
                "Isometric Angle",
            ...
            var isometricOptions = new[]
            {
                ...
                new GUIContent("Dimetric (60°)"),
                new GUIContent("Custom")
            };
            var isometricValues = new[]
            {
                0f,
                GridGraph.StandardIsometricAngle,
                GridGraph.StandardDimetricAngle
            };
            ...
            isometricOption = EditorGUILayout.IntPopup(
                isometricGUIContent,
                isometricOption,
                isometricOptions,
                new[] { 0, 1, 2, 3 }
            ...
            else
            {
                ...
                // Custom
                graph.isometricAngle = EditorGUILayout.FloatField(
                    isometricGUIContent,
                    graph.isometricAngle
                );
                EditorGUI.indentLevel--;
            }
        }
        ...
        void DrawPositionField(GridGraph graph)
        {
            GUILayout.BeginHorizontal();
            var normalizedPivotPoint = NormalizedPivotPoint(graph, pivot);
            var worldPoint = RoundVector3(
                graph.CalculateTransform().Transform(normalizedPivotPoint)
            );
            var newWorldPoint = EditorGUILayout.Vector3Field(
                ObjectNames.NicifyVariableName(pivot.ToString()),
                worldPoint
            );
        ...
        protected virtual void DrawCutCorners(GridGraph graph)
        {
            ...
            graph.cutCorners = EditorGUILayout.Toggle(
                new GUIContent(
                    "Cut Corners",
                    "Enables or disables cutting corners. See docs for image example"
                ),
                graph.cutCorners
            );
        }
        ...
        protected virtual void DrawNeighbours(GridGraph graph)
        {
            ...
            var neighboursGUIContent = new GUIContent(
                "Connections",
            ...
            if (graph.inspectorGridMode == InspectorGridMode.Advanced)
            {
                neighbourOptions = new[]
                {
                    new GUIContent("Four"),
                    new GUIContent("Eight"),
                    new GUIContent("Six")
                };
            }
            else
            ...
            graph.neighbours = (NumNeighbours)
                EditorGUILayout.Popup(
                    neighboursGUIContent,
                    (int)graph.neighbours,
                    neighbourOptions
                );
            ...
            {
                graph.uniformEdgeCosts = EditorGUILayout.Toggle(
                    new GUIContent(
                        "Hexagon connection costs",
        ...
        protected void DrawMaxSlope(GridGraph graph)
        {
            if (!graph.collision.use2D)
            {
                graph.maxSlope = EditorGUILayout.Slider(
                    new GUIContent(
                        "Max Slope",
        ...
        {
            graph.erodeIterations = EditorGUILayout.IntField(
                new GUIContent(
                    "Erosion iterations",
            ...
            {
                EditorGUI.indentLevel++;
                graph.erosionUseTags = EditorGUILayout.Toggle(
                    new GUIContent(
                        "Erosion Uses Tags",
                ...
                {
                    EditorGUI.indentLevel++;
                    graph.erosionFirstTag = EditorGUILayoutx.TagField(
                        "First Tag",
                        graph.erosionFirstTag,
        ...
        void DrawLastSection(GridGraph graph)
        {
            GUILayout.BeginHorizontal();
            GUILayout.Space(18);
            graph.showMeshSurface = GUILayout.Toggle(
                graph.showMeshSurface,
                new GUIContent(
                    "Show surface",
                    "Toggles gizmos for drawing the surface of the mesh"
                ),
                EditorStyles.miniButtonLeft
            );
            graph.showMeshOutline = GUILayout.Toggle(
                graph.showMeshOutline,
                new GUIContent(
                    "Show outline",
                    "Toggles gizmos for drawing an outline of the nodes"
                ),
                EditorStyles.miniButtonMid
            );
            graph.showNodeConnections = GUILayout.Toggle(
                graph.showNodeConnections,
        ...
        void DrawPenaltyModifications(GridGraph graph)
        {
            ...
            if (showExtra)
            {
                ...
                graph.penaltyAngle = ToggleGroup(
                    new GUIContent(
                        "Angle Penalty",
                        "Adds a penalty based on the slope of the node"
                    ),
                    graph.penaltyAngle
                );
                if (graph.penaltyAngle)
                {
                    ...
                    graph.penaltyAngleFactor = EditorGUILayout.FloatField(
                        new GUIContent(
                            "Factor",
                            "Scale of the penalty. A negative value should not be used"
                        ),
                        graph.penaltyAngleFactor
                    );
                    graph.penaltyAnglePower = EditorGUILayout.Slider(
                        "Power",
                        graph.penaltyAnglePower,
                        0.1f,
                        10f
                    );
                    EditorGUILayout.HelpBox(
                ...
                {
                    EditorGUI.indentLevel++;
                    graph.penaltyPositionOffset = EditorGUILayout.FloatField(
                        "Offset",
                        graph.penaltyPositionOffset
                    );
                    graph.penaltyPositionFactor = EditorGUILayout.FloatField(
                        "Factor",
                        graph.penaltyPositionFactor
                    );
                    EditorGUILayout.HelpBox(
        ...
        protected virtual void DrawCollisionEditor(GraphCollision collision)
        {
            ...
            {
                string[] colliderOptions = collision.use2D
                    ? new[] { "Circle", "Point" }
                    : new[] { "Sphere", "Capsule", "Ray" };
                ...
                if (collision.type == ColliderType.Capsule || collision.type == ColliderType.Sphere)
                {
                    collision.diameter = EditorGUILayout.FloatField(
                        new GUIContent(
                            "Diameter",
                            "Diameter of the capsule or sphere. 1 equals one node width"
                        ),
                        collision.diameter
                    );
                }
                ...
                {
                    if (
                        collision.type == ColliderType.Capsule || collision.type == ColliderType.Ray
                    )
                    {
                        collision.height = EditorGUILayout.FloatField(
                            new GUIContent(
                                "Height/Length",
                                "Height of cylinder or length of ray in world units"
                            ),
                            collision.height
                        );
                    }
                ...
                collision.mask = EditorGUILayoutx.LayerMaskField(
                    "Obstacle Layer Mask",
                    collision.mask
                );
            ...
            else
            {
                ...
                if (collision.heightCheck)
                {
                    ...
                    collision.heightMask = EditorGUILayoutx.LayerMaskField(
                        "Mask",
                        collision.heightMask
                    );
        ...
        void DrawCollisionPreview(GraphCollision collision)
        {
            ...
            interpolatedGridWidthInNodes = Mathf.Lerp(
            ...
            var diameter =
                collision.type == ColliderType.Ray ? 0.05f : collision.diameter * nodeSize;
            ...
            if (collision.type == ColliderType.Sphere)
            {
                ...
                sideScale = Mathf.Min(
                    sideScale,
                    sideBase.y / (Mathf.Max(0, collision.collisionOffset) + diameter)
                );
            }
            else
            {
                sideScale = Mathf.Max(
                    ...
                    Mathf.Min(
                        scale,
                        sideBase.y
                            / (collision.height + collision.collisionOffset + diameter * 0.5f)
                    )
                );
                // A high collision offset should not cause it to break
                sideScale = Mathf.Min(
            ...
            for (int i = 0; i <= gridWidthInNodes; i++)
            {
                var c = i * 1.0f / gridWidthInNodes;
                DrawArc(
                    sideBase + new Vector2(c - 0.5f, 0) * interpolatedGridSideScale,
                    2,
                    0,
                    Mathf.PI * 2
                );
            ...
            if (collision.type == ColliderType.Ray)
            {
                ...
                var maxHeight =
                    sideBase.y / sideScale - (collision.collisionOffset + diameter * 0.5f);
                float dashLength = 0;
                ...
                DrawDashedLine(
                    offset + new Vector2(0, -height) * sideScale,
                    offset + new Vector2(0, -height * 0.75f) * sideScale,
                    dashLength
                );
            ...
            else
            {
                ...
                DrawArc(
                    sideBase + new Vector2(0, -collision.collisionOffset * sideScale),
                    diameter * 0.5f * sideScale,
                    0,
                    Mathf.PI
                );
                DrawArc(
                    sideBase + new Vector2(0, -(height + collision.collisionOffset) * sideScale),
                    diameter * 0.5f * sideScale,
                    Mathf.PI,
                    2 * Mathf.PI
                );
                DrawDashedLine(
                    sideBase
                        + new Vector2(-diameter * 0.5f, -collision.collisionOffset) * sideScale,
                    sideBase
                        + new Vector2(-diameter * 0.5f, -(height + collision.collisionOffset))
                            * sideScale,
                    dashLength
                );
                DrawDashedLine(
                    sideBase + new Vector2(diameter * 0.5f, -collision.collisionOffset) * sideScale,
                    sideBase
                        + new Vector2(diameter * 0.5f, -(height + collision.collisionOffset))
                            * sideScale,
                    dashLength
        ...
        {
            collision.use2D = EditorGUILayout.Toggle(
                new GUIContent("Use 2D Physics", "Use the Physics2D API for collision checking"),
                collision.use2D
            );
        }
        ...
        public static GridPivot PivotPointSelector(GridPivot pivot)
        {
            ...
            gridPivotSelectButton =
                gridPivotSelectButton
                ?? AstarPathEditor.astarSkin.FindStyle("GridPivotSelectButton");
        ...
        public override void OnSceneGUI(NavGraph target)
        {
            ...
            if (Tools.current == Tool.Scale)
            {
                ...
                for (int i = 0; i < handlePoints.Length; i++)
                {
                    ...
                    Vector3 p = savedTransform.InverseTransform(
                        Handles.Slider(
                            ps,
                            ps - center,
                            HandleScale * HandleUtility.GetHandleSize(ps),
                            cap,
                            0
                        )
                    );
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    graph.unclampedSize =
                        Vector2.Scale(new Vector2(mx.x - mn.x, mx.z - mn.z), savedDimensions)
                        * savedNodeSize;
                }
            }
