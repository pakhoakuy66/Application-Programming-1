    ...
    public class GraphEditor : GraphEditorBase
    {
        ...
        public bool AutoScan()
        {
            if (
                ...
                && AstarPath.active.lastScanTime < 0.11F
            )
            {
            ...
            if (obj != null)
            {
                if (allowSceneObjects && !EditorUtility.IsPersistent(obj))
                {
                    ...
                    if (go != null && go.GetComponent<UnityReferenceHelper>() == null)
                    {
                        if (
                            FixLabel(
                                "Object's GameObject must have a UnityReferenceHelper component attached"
                            )
                        )
                        {
                ...
                else if (EditorUtility.IsPersistent(obj))
                {
                    if (assetsMustBeInResourcesFolder)
                    {
                        ...
                        if (!rg.IsMatch(path))
                        {
                            ...
                            {
                                if (
                                    !System.IO.Directory.Exists(Application.dataPath + "/Resources")
                                )
                                {
                        ...
                        if (
                            ...
                            && obj.name != AssetDatabase.LoadMainAssetAtPath(path).name
                        )
                        {
                            if (
                                FixLabel(
        ...
        public void OnBaseInspectorGUI(NavGraph target)
        {
            int penalty = EditorGUILayout.IntField(
                new GUIContent(
                    "Initial Penalty",
                    "Initial Penalty for nodes in this graph. Set during Scan."
                ),
        ...
        {
            GUIStyle separator =
                AstarPathEditor.astarSkin.FindStyle("PixelBox3Separator") ?? new GUIStyle();
        ...
        public static bool FixLabel(string label, string buttonLabel = "Fix", int buttonWidth = 40)
        {
            ...
            GUILayout.Label(
                label,
                EditorGUIUtility.isProSkin ? EditorStyles.whiteMiniLabel : EditorStyles.miniLabel,
                GUILayout.ExpandWidth(true)
            );
            var returnValue = GUILayout.Button(
                buttonLabel,
