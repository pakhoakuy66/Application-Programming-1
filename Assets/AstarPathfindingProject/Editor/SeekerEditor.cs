...
namespace Pathfinding
{
    ...
    public class SeekerEditor : EditorBase
    {
        ...
        {
            new GUIContent("Node Center (Snap To Node)"),
            new GUIContent("Original"),
            new GUIContent("Interpolate (deprecated)"),
            new GUIContent("Closest On Node Surface"),
        ...
        protected override void Inspector()
        {
            ...
            startEndModifierProp.isExpanded = EditorGUILayout.Foldout(
                startEndModifierProp.isExpanded,
                startEndModifierProp.displayName
            );
            if (startEndModifierProp.isExpanded)
            {
                ...
                if (
                    FindProperty("startEndModifier.exactStartPoint").enumValueIndex
            ...
            for (int i = 0; i < graphLabels.Length; i++)
            {
                if (
                    ...
                    || AstarPath.active.data.graphs[i] == null
                )
            ...
            tagPenaltiesOpen = EditorGUILayout.Foldout(
                tagPenaltiesOpen,
                new GUIContent("Tags", "Settings for each tag")
            );
            if (tagPenaltiesOpen)
            {
                ...
                {
                    if (
                        scripts[i].tagPenalties == null
                        || scripts[i].tagPenalties.Length != tagNames.Length
                    )
                ...
                if (GUILayout.Button("Set all/none", EditorStyles.miniButton))
                {
                    ...
                    {
                        scripts[j].traversableTags =
                            (scripts[0].traversableTags & 0x1) == 0 ? -1 : 0;
                    }
                }
                EditorGUILayout.EndVertical();
