    ...
    public class AstarPathEditor : Editor
    {
        ...
        void CreateFadeAreas()
        {
            if (settingsArea == null)
            {
                ...
                optimizationSettingsArea = new FadeArea(
                    false,
                    this,
                    level0AreaStyle,
                    level0LabelStyle
                ...
                serializationSettingsArea = new FadeArea(
                    false,
                    this,
                    level0AreaStyle,
                    level0LabelStyle
        ...
        static bool IsJsEnabled()
        {
            return System.IO.Directory.Exists(
                Application.dataPath + "/AstarPathfindingEditor/Editor"
            );
        }
        ...
        static void EnableJs()
        {
            ...
            if (!System.IO.Directory.Exists(Application.dataPath + "/AstarPathfindingEditor"))
            {
                System.IO.Directory.CreateDirectory(
                    Application.dataPath + "/AstarPathfindingEditor"
                );
            ...
            AssetDatabase.MoveAsset(
                scriptsFolder + "/Editor",
                "Assets/AstarPathfindingEditor/Editor"
            );
        ...
        static void DisableJs()
        {
            if (
                System.IO.Directory.Exists(
                    Application.dataPath + "/Plugins/AstarPathfindingProject"
                )
            )
            {
                string error = AssetDatabase.MoveAsset(
                    "Assets/Plugins/AstarPathfindingProject",
                    scriptsFolder
                );
                if (error != "")
                {
            ...
            {
                Debug.LogWarning(
                    "Could not disable JS - Could not find directory '"
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!LoadStyles())
            {
                EditorGUILayout.HelpBox(
                    "The GUISkin 'AstarEditorSkin.guiskin' in the folder "
...
#if ASTAR_ATAVISM
            EditorGUILayout.HelpBox(
                "This is a special version of the A* Pathfinding Project for Atavism. This version only supports scanning recast graphs and exporting them, but no pathfinding during runtime.",
                MessageType.Info
            );
#endif
        ...
        void DrawAboutArea()
        {
            ...
            if (
                FullyDefinedVersion(AstarUpdateChecker.latestVersion)
                    > FullyDefinedVersion(AstarPath.Version)
                || FullyDefinedVersion(AstarUpdateChecker.latestBetaVersion)
                    > FullyDefinedVersion(AstarPath.Version)
            )
            {
                if (
            ...
            if (FullyDefinedVersion(newVersion) > FullyDefinedVersion(AstarPath.Version))
            {
                ...
                if (
                    GUILayout.Button(
                        (beta ? "Beta" : "New") + " Version Available! " + newVersion,
                        thinHelpBox,
                        GUILayout.Height(15)
                    )
                )
            ...
            if (aboutArea.BeginFade())
            {
                ...
                if (FullyDefinedVersion(newVersion) > FullyDefinedVersion(AstarPath.Version))
                {
                    ...
                    {
                        Application.OpenURL(
                            AstarUpdateChecker.GetURL(beta ? "beta_changelog" : "changelog")
                        );
                    }
        ...
        void DrawGraphHeader(GraphEditor graphEditor)
        {
            ...
            if (graphNameFocused == graphEditor)
            {
                ...
                graph.name = GUILayout.TextField(
                    graph.name ?? "",
                    level1LabelStyle,
                ...
                if (
                    (
                        ...
                        && GUI.GetNameOfFocusedControl() != graphGUIDString
                    )
                    || (
            ...
            {
                var moveUp = GUILayout.Button(
                    new GUIContent("Up", "Increase the graph priority"),
                    GUILayout.Width(40)
                );
                var moveDown = GUILayout.Button(
                    new GUIContent("Down", "Decrease the graph priority"),
                    GUILayout.Width(40)
                );
            ...
            graph.drawGizmos = GUILayout.Toggle(
                graph.drawGizmos,
                new GUIContent("Draw Gizmos", "Draw Gizmos"),
                graphGizmoButtonStyle
            );
        ...
        void DrawGraphInfoArea(GraphEditor graphEditor)
        {
            ...
            if (graphEditor.infoFadeArea.BeginFade())
            {
                ...
                if (
                    !graphNodeCounts.TryGetValue(graphEditor.target, out pair)
                ...
                )
                {
                    ...
                    pair = new KeyValuePair<float, KeyValuePair<int, int>>(
                ...
                if (total == 0)
                    EditorGUILayout.HelpBox(
                        "The number of nodes in the graph is zero. The graph might not be scanned",
                        MessageType.Info
                    );
        ...
        void DrawSceneGUISettings()
        {
            ...
            GUILayout.BeginArea(
                new Rect(
                    Camera.current.pixelWidth - width,
                    Camera.current.pixelHeight - height,
                    width - margin,
                    height - margin
                ),
                "Graph Display",
                astarSkin.FindStyle("SceneBoxDark")
            );
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.PrefixLabel(
                "Show Graphs",
                darkSkin.toggle,
                astarSkin.FindStyle("ScenePrefixLabel")
            );
        ...
        TextAsset SaveGraphData(byte[] bytes, TextAsset target = null)
        {
            ...
            else
            {
                ...
                {
                    path =
                        "Assets/GraphCaches/GraphCache" + (i == 0 ? "" : i.ToString()) + ".bytes";
                    i++;
        ...
        void DrawSerializationSettings()
        {
            ...
            if (serializationSettingsArea.BeginFade())
            {
                ...
                if (GUILayout.Button("Generate cache"))
                {
                    ...
                    // Store it in a file
                    script.data.file_cachedStartup = SaveGraphData(
                        bytes,
                        script.data.file_cachedStartup
                    );
                ...
                if (GUILayout.Button("Load from cache"))
                {
                    if (
                        EditorUtility.DisplayDialog(
                            "Are you sure you want to load from cache?",
                ...
                if (
                    ...
                    && script.data.data_cachedStartup.Length > 0
                )
                {
                    EditorGUILayout.HelpBox(
                    ...
                    {
                        script.data.file_cachedStartup = SaveGraphData(
                            script.data.data_cachedStartup
                        );
                        script.data.data_cachedStartup = null;
                    }
                }
                ...
                if (GUILayout.Button("Save to file"))
                {
                    ...
                    if (path != "")
                    {
                        var serializationSettings = Pathfinding
                            .Serialization
                            .SerializeSettings
                            .Settings;
                        if (
                ...
                if (GUILayout.Button("Load from file"))
                {
                    ...
                    if (path != "")
                    {
                        try
                        {
                            byte[] bytes = Pathfinding.Serialization.AstarSerializer.LoadFromFile(
                                path
                            );
        ...
        void DrawPathfindingSettings()
        {
            ...
            script.threadCount = (ThreadCount)
            ...
            if (threads > 0)
                ...
                EditorGUILayout.HelpBox(
                    "Using a single coroutine (no threads)"
                        + (script.threadCount < 0 ? " on your machine" : ""),
                    MessageType.None
                );
            ...
            if (script.threadCount == ThreadCount.None)
            {
                script.maxFrameTime = EditorGUILayout.FloatField(
                    new GUIContent(
                        "Max Frame Time",
                        "Max number of milliseconds to use for path calculation per frame"
                    ),
                    script.maxFrameTime
                );
            }
            ...
            script.maxNearestNodeDistance = EditorGUILayout.FloatField(
                new GUIContent(
            ...
            if (
                script.heuristic == Heuristic.Manhattan
                || script.heuristic == Heuristic.Euclidean
                || script.heuristic == Heuristic.DiagonalManhattan
            )
            {
                EditorGUI.indentLevel++;
                script.heuristicScale = EditorGUILayout.FloatField(
                    "Heuristic Scale",
                    script.heuristicScale
                );
                EditorGUI.indentLevel--;
            }
            ...
            if (script.data.FindGraphWhichInheritsFrom(typeof(NavmeshBase)) != null)
            {
                script.navmeshUpdates.updateInterval = EditorGUILayout.FloatField(
                    new GUIContent(
                        "Navmesh Cutting Update Interval (s)",
                        "How often to check if any navmesh cut has changed."
                    ),
                    script.navmeshUpdates.updateInterval
                );
            }

            script.prioritizeGraphs = EditorGUILayout.Toggle(
                new GUIContent(
                    "Prioritize Graphs",
            ...
            {
                EditorGUI.indentLevel++;
                script.prioritizeGraphsLimit = EditorGUILayout.FloatField(
                    "Priority Limit",
            ...
            script.fullGetNearestSearch = EditorGUILayout.Toggle(
                new GUIContent(
        ...
        void DrawTagSettings()
        {
            ...
            if (tagsArea.BeginFade())
            {
                ...
                {
                    tagNames[i] = EditorGUILayout.TextField(
                        new GUIContent("Tag " + i, "Name for tag " + i),
                        tagNames[i]
                    );
                    if (tagNames[i] == "")
                        tagNames[i] = "" + i;
        ...
        void DrawEditorSettings()
        {
            ...
            {
                FadeArea.fancyEffects = EditorGUILayout.Toggle(
                    "Smooth Transitions",
                    FadeArea.fancyEffects
                );
        ...
        void DrawDebugSettings()
        {
            ...
            script.logPathResults = (PathLog)
                EditorGUILayout.EnumPopup("Path Logging", script.logPathResults);
            script.debugMode = (GraphDebugMode)
                EditorGUILayout.EnumPopup("Graph Coloring", script.debugMode);
            ...
            if (
                script.debugMode == GraphDebugMode.G
                || script.debugMode == GraphDebugMode.H
                || script.debugMode == GraphDebugMode.F
                || script.debugMode == GraphDebugMode.Penalty
            )
            {
                script.manualDebugFloorRoof = !EditorGUILayout.Toggle(
                    "Automatic Limits",
                    !script.manualDebugFloorRoof
                );
                DrawColorSlider(
                    ref script.debugFloor,
                    ref script.debugRoof,
                    script.manualDebugFloorRoof
                );
            }

            script.showSearchTree = EditorGUILayout.Toggle(
                "Show Search Tree",
                script.showSearchTree
            );
            if (script.showSearchTree)
            {
                EditorGUILayout.HelpBox(
                    "Show Search Tree is enabled, you may see rendering glitches in the graph rendering"
                        ...
                        + " are being rendered. You can pause the game to see an accurate rendering.",
                    MessageType.Info
                );
            }
            script.showUnwalkableNodes = EditorGUILayout.Toggle(
                "Show Unwalkable Nodes",
                script.showUnwalkableNodes
            );

            if (script.showUnwalkableNodes)
            {
                EditorGUI.indentLevel++;
                script.unwalkableNodeDebugSize = EditorGUILayout.FloatField(
                    "Size",
                    script.unwalkableNodeDebugSize
                );
                EditorGUI.indentLevel--;
            }
        ...
        void DrawColorSettings()
        {
            ...
            if (colorSettingsArea.BeginFade())
            {
                ...
                colors._UnwalkableNode = EditorGUILayout.ColorField(
                    "Unwalkable Node",
                    colors._UnwalkableNode
                );
                colors._BoundsHandles = EditorGUILayout.ColorField(
                    "Bounds Handles",
                    colors._BoundsHandles
                );

                colors._ConnectionLowLerp = EditorGUILayout.ColorField(
                    "Connection Gradient (low)",
                    colors._ConnectionLowLerp
                );
                colors._ConnectionHighLerp = EditorGUILayout.ColorField(
                    "Connection Gradient (high)",
                    colors._ConnectionHighLerp
                );

                colors._MeshEdgeColor = EditorGUILayout.ColorField(
                    "Mesh Edge",
                    colors._MeshEdgeColor
                );

                if (
                    EditorResourceHelper.GizmoSurfaceMaterial != null
                    && EditorResourceHelper.GizmoLineMaterial != null
                )
                {
                    EditorGUI.BeginChangeCheck();
                    ...
                    if (EditorGUI.EndChangeCheck())
                    {
                        Undo.RecordObjects(
                            new[]
                            {
                                EditorResourceHelper.GizmoSurfaceMaterial,
                                EditorResourceHelper.GizmoLineMaterial
                            },
                        ...
                        EditorResourceHelper.GizmoLineMaterial.SetColor(
                            "_FadeColor",
                            fade * new Color(1, 1, 1, 0.7f)
                        );
                    }
                }
                ...
                // Custom Area Colors
                customAreaColorsOpen = EditorGUILayout.Foldout(
                ...
                if (customAreaColorsOpen)
                {
                    ...
                    {
                        GUILayout.BeginHorizontal();
                        colors._AreaColors[i] = EditorGUILayout.ColorField(
                            "Area " + i + (i == 0 ? " (not used usually)" : ""),
                            colors._AreaColors[i]
                        );
                        if (
                            GUILayout.Button(
                                new GUIContent("", "Reset to the default color"),
                                astarSkin.FindStyle("SmallReset"),
                                GUILayout.Width(20)
                            )
                        )
                        {
        ...
        void CheckGraphEditors(bool forceRebuild = false)
        {
            ...
            else
            {
                for (int i = 0; i < script.graphs.Length; i++)
                {
                    ...
                    if (
                        graphEditors[i] == null
                        || graphEditorTypes[script.graphs[i].GetType().Name].editorType
        ...
        GraphEditor CreateGraphEditor(NavGraph graph)
        {
            ...
            {
                result =
                    System.Activator.CreateInstance(graphEditorTypes[graphType].editorType)
                    as GraphEditor;
            }
            else
            {
                Debug.LogError(
                    "Couldn't find an editor for the graph type '"
                        + graphType
                        + "' There are "
                        + graphEditorTypes.Count
                        + " available graph editors"
                );
        ...
        public void SaveGraphsAndUndo(EventType et = EventType.Used, string eventCommand = "")
        {
            ...
            if (
                (Undo.GetCurrentGroup() != lastUndoGroup || et == EventType.MouseUp)
        ...
        )
        {
            ...
            for (int i = 0; i < graphEditors.Length; i++)
            {
                ...
                (graphEditors[i].target as IGraphInternals).SerializedEditorSettings =
                    output.ToString();
            }
        ...
        void DeserializeGraphs(byte[] bytes)
        {
            try
            {
                ...
                for (int i = 0; i < graphEditors.Length; i++)
                {
                    ...
                    if (data != null)
                        Pathfinding.Serialization.TinyJsonDeserializer.Deserialize(
                            data,
                            graphEditors[i].GetType(),
                            graphEditors[i],
                            script.gameObject
        ...
        public static void MenuScan()
        {
            ...
            catch (System.Exception e)
            {
                ...
                EditorUtility.DisplayDialog(
                    ...
                    "There was an error when generating graphs, check the console for more info",
                    "Ok"
                );
                throw e;
            }
