...
namespace Pathfinding
{
    ...
    public class SimpleSmoothModifier : MonoModifier
    {
        ...
        public List<Vector3> SmoothOffsetSimple(List<Vector3> path)
        {
            ...
            {
                Debug.LogWarning(
                    "A very high iteration count was passed, won't let this one through"
                );
                return path;
            }
            ...
            for (int iteration = 0; iteration < iterations; iteration++)
            {
                ...
                for (int i = 0; i < currentPathLength - 1; i++)
                {
                    ...
                    {
                        setFirst = true;
                        firstRight = VectorMath.RightOrColinearXZ(
                            current,
                            next,
                            subdivided2[i - 1]
                        );
                    ...
                    if (
                        i < currentPathLength - 1
                        && !VectorMath.IsColinearXZ(current, next, subdivided2[i + 2])
                    )
                    {
                        setSecond = true;
                        secondRight = VectorMath.RightOrColinearXZ(
                            current,
                            next,
                            subdivided2[i + 2]
                        );
            ...
            if (strength > 0)
            {
                for (int it = 0; it < iterations; it++)
                {
                    ...
                    for (int i = 1; i < subdivided.Count - 1; i++)
                    {
                        ...
                        subdivided[i] = Vector3.Lerp(
                            tmp,
                            (prev + subdivided[i + 1]) / 2F,
                            strength
                        );
