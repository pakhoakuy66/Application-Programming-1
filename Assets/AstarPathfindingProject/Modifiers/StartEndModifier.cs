...
namespace Pathfinding
{
    ...
    public class StartEndModifier : PathModifier
    {
        ...
        public override void Apply(Path _p)
        {
            ...
            int closestStartConnection,
                closestEndConnection;

            Vector3 pStart = Snap(
                p,
                exactStartPoint,
                true,
                out forceAddStartPoint,
                out closestStartConnection
            );
            Vector3 pEnd = Snap(
                p,
                exactEndPoint,
                false,
                out forceAddEndPoint,
                out closestEndConnection
            );
        ...
        )
        {
            ...
            switch (mode)
            {
                ...
                case Exactness.NodeConnection:
                    ...
                    {
                        relevantPoint =
                            adjustStartPoint != null ? adjustStartPoint() : path.originalStartPoint;
                    }
                    else
                    {
                    ...
                    switch (mode)
                    {
                        ...
                        case Exactness.Interpolate:
                            ...
                            var adjacentNode = path.path[
                                Mathf.Clamp(index + (start ? 1 : -1), 0, path.path.Count - 1)
                            ];
                            return VectorMath.ClosestPointOnSegment(
                                nodePos,
                                (Vector3)adjacentNode.position,
                                relevantPoint
                            );
                        case Exactness.NodeConnection:
                            ...
                            for (int i = connectionBuffer.Count - 1; i >= 0; i--)
                            {
                                ...
                                var closest = VectorMath.ClosestPointOnSegment(
                                    nodePos,
                                    (Vector3)neighbour.position,
                                    relevantPoint
                                );
