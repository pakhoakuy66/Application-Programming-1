...
namespace Pathfinding.Examples
{
    ...
    [HelpURL(
        "http://arongranberg.com/astar/docs/class_pathfinding_1_1_examples_1_1_procedural_world.php"
    )]
    public class ProceduralWorld : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            Int2 p = new Int2(
                Mathf.RoundToInt((target.position.x - tileSize * 0.5f) / tileSize),
            ...
            // make sure they are
            for (
                int x = p.x - disableAsyncLoadWithinRange;
                x <= p.x + disableAsyncLoadWithinRange;
                x++
            )
            {
                for (
                    int z = p.y - disableAsyncLoadWithinRange;
                    z <= p.y + disableAsyncLoadWithinRange;
                    z++
                )
                {
                ...
                for (int i = 0; i < world.prefabs.Length; i++)
                {
                    ...
                    if (pref.singleFixed)
                    {
                        Vector3 p = new Vector3(
                            (x + 0.5f) * world.tileSize,
                            0,
                            (z + 0.5f) * world.tileSize
                        );
                        GameObject ob =
                            GameObject.Instantiate(pref.prefab, p, Quaternion.identity)
                            as GameObject;
                    ...
                    else
                    {
                        ...
                        for (int sx = 0; sx < world.subTiles; sx++)
                        {
                            for (int sz = 0; sz < world.subTiles; sz++)
                            {
                                ...
                                float perl = Mathf.Pow(
                                    Mathf.PerlinNoise(
                                        (px + pref.perlinOffset.x) * pref.perlinScale,
                                        (pz + pref.perlinOffset.y) * pref.perlinScale
                                    ),
                                    pref.perlinPower
                                );
                                ...
                                float density =
                                    pref.density
                                    * Mathf.Lerp(1, perl, pref.perlin)
                                    * Mathf.Lerp(1, (float)rnd.NextDouble(), pref.random);
                                float fcount =
                                    subSize * subSize * density + ditherMap[sx + 1, sz + 1];
                                ...
                                for (int j = 0; j < count; j++)
                                {
                                    ...
                                    GameObject ob =
                                        GameObject.Instantiate(pref.prefab, p, RandomYRot(pref))
                                        as GameObject;
                                    ob.transform.parent = root;
                                    //ob.SetActive ( false );
                                    //objs.Add ( ob );
