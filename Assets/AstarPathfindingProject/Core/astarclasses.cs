...
namespace Pathfinding
{
    ...
    public class AstarColor
    {
        ...
        {
            var hash =
                SolidColor.GetHashCode()
                ^ UnwalkableNode.GetHashCode()
                ^ BoundsHandles.GetHashCode()
                ^ ConnectionLowLerp.GetHashCode()
                ^ ConnectionHighLerp.GetHashCode()
                ^ MeshEdgeColor.GetHashCode();

            for (int i = 0; i < AreaColors.Length; i++)
                hash = 7 * hash ^ AreaColors[i].GetHashCode();
            return hash;
        }
    ...
    public class NNConstraint
    {
        ...
        public virtual bool Suitable(GraphNode node)
        {
            ...
            if (constrainTags && ((tags >> (int)node.Tag) & 0x1) == 0)
                return false;
    ...
    public class PathNNConstraint : NNConstraint
    {
        ...
        {
            get { return new PathNNConstraint { constrainArea = true }; }
        }
    ...
    public struct NNInfoInternal
    {
        ...
        public void UpdateInfo()
        {
            ...
            constClampedPosition =
                constrainedNode != null ? (Vector3)constrainedNode.position : Vector3.zero;
        }
    }
        ...
        public static GraphMask FromGraphName(string graphName)
        {
            ...
            if (graph == null)
                throw new System.ArgumentException(
                    "Could not find any graph with the name '" + graphName + "'"
                );
