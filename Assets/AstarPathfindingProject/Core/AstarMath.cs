...
namespace Pathfinding
{
    ...
    public static class VectorMath
    {
        ...
        /// See: ClosestPointOnSegment
        /// </summary>
        public static float ClosestPointOnLineFactor(
            Vector3 lineStart,
            Vector3 lineEnd,
            Vector3 point
        )
        ...
        )
        {
            ...
            return lineStart
                + (Mathf.Clamp(closestPoint, 0.0f, fullDirection2.magnitude) * lineDirection);
        }
        ...
        /// </summary>
        public static float SqrDistanceSegmentSegment(
            Vector3 s1,
            Vector3 e1,
            Vector3 s2,
            Vector3 e2
        ...
        {
            return (long)(b.x - a.x) * (long)(p.z - a.z) - (long)(p.x - a.x) * (long)(b.z - a.z)
                < 0;
        }
        ...
        {
            return (long)(b.x - a.x) * (long)(p.y - a.y) - (long)(p.x - a.x) * (long)(b.y - a.y)
                <= 0;
        }
        ...
        public static bool SegmentIntersectsBounds(Bounds bounds, Vector3 a, Vector3 b)
        {
            ...
            if (Math.Abs(LMid.x) > extent.x + LExt.x)
                return false;
            if (Math.Abs(LMid.y) > extent.y + LExt.y)
                return false;
            if (Math.Abs(LMid.z) > extent.z + LExt.z)
                return false;
            // Crossproducts of line and each axis
            if (Math.Abs(LMid.y * L.z - LMid.z * L.y) > (extent.y * LExt.z + extent.z * LExt.y))
                return false;
            if (Math.Abs(LMid.x * L.z - LMid.z * L.x) > (extent.x * LExt.z + extent.z * LExt.x))
                return false;
            if (Math.Abs(LMid.x * L.y - LMid.y * L.x) > (extent.x * LExt.y + extent.y * LExt.x))
                return false;
    ...
    public static class Polygon
    {
        ...
        {
            return VectorMath.IsClockwiseMarginXZ(a, b, p)
                && VectorMath.IsClockwiseMarginXZ(b, c, p)
        ...
        {
            return VectorMath.IsClockwiseOrColinearXZ(a, b, p)
                && VectorMath.IsClockwiseOrColinearXZ(b, c, p)
        ...
        {
            return VectorMath.IsClockwiseOrColinear(a, b, p)
                && VectorMath.IsClockwiseOrColinear(b, c, p)
        ...
        public static bool ContainsPointXZ(Vector3[] polyPoints, Vector3 p)
        {
            ...
            for (int i = 0; i < polyPoints.Length; j = i++)
            {
                if (
                    (
                        (polyPoints[i].z <= p.z && p.z < polyPoints[j].z)
        ...
        {
            double det =
                ((double)(p2.z - p3.z)) * (p1.x - p3.x) + ((double)(p3.x - p2.x)) * (p1.z - p3.z);

            double lambda1 = (
                (((double)(p2.z - p3.z)) * (p.x - p3.x) + ((double)(p3.x - p2.x)) * (p.z - p3.z))
                / det
            );
            double lambda2 = (
                (((double)(p3.z - p1.z)) * (p.x - p3.x) + ((double)(p1.x - p3.x)) * (p.z - p3.z))
                / det
            );

            return (int)
                Math.Round(lambda1 * p1.y + lambda2 * p2.y + (1 - lambda1 - lambda2) * p3.y);
        }
        ...
        )
        {
            ...
            for (int i = 0; i < vertices.Count; i++)
            {
                ...
                int ind;
                if (
                    !firstVerts.TryGetValue(vertices[i], out ind)
        ...
        public static void TraceContours(
            Dictionary<int, int> outline,
