...
namespace Pathfinding
{
    ...
    public class AstarDebugger : VersionedMonoBehaviour
    {
        ...
        PathTypeDebug[] debugTypes = new PathTypeDebug[]
        {
            new PathTypeDebug(
                "ABPath",
                () => PathPool.GetSize(typeof(ABPath)),
        ...
        public void LateUpdate()
        {
            ...
            if (Application.isPlaying && cam != null && showGraph)
            {
                ...
                float minMem = float.PositiveInfinity,
                    maxMem = 0,
                ...
                Matrix4x4 m = Matrix4x4.TRS(
                    new Vector3((cam.pixelWidth - graphWidth) / 2f, graphOffset, 1),
                    Quaternion.identity,
                    new Vector3(graphWidth, graphHeight, 1)
                );
                ...
                for (int i = 0; i < graph.Length - 1; i++)
                {
                    ...
                    DrawGraphLine(
                        i,
                        m,
                        i / (float)graph.Length,
                        (i + 1) / (float)graph.Length,
                        Mathf.InverseLerp(minMem, maxMem, graph[i].memory),
                        Mathf.InverseLerp(minMem, maxMem, graph[i + 1].memory),
                        Color.blue
                    );
                    DrawGraphLine(
                        i,
                        ...
                        Mathf.InverseLerp(minFPS, maxFPS, graph[i + 1].fps),
                        Color.green
                    );
                }
            }
        ...
        {
            Debug.DrawLine(
                cam.ScreenToWorldPoint(m.MultiplyPoint3x4(new Vector3(x1, y1))),
                cam.ScreenToWorldPoint(m.MultiplyPoint3x4(new Vector3(x2, y2))),
                color
            );
        }
        ...
        public void OnGUI()
        {
            ...
            if (
                Time.realtimeSinceStartup - lastUpdate > 0.5f
                || cachedText == null
            ...
            )
            {
                ...
                if (showPathProfile)
                {
                    ...
                    else
                    {
#if ProfileAstar
                        double searchSpeed =
                            (double)AstarPath.TotalSearchedNodes
                            * 10000
                            / (double)AstarPath.TotalSearchTime;
                        text.Append("\nSearch Speed	(nodes/ms)	")
                            .Append(searchSpeed.ToString("0"))
                            .Append(" (" + AstarPath.TotalSearchedNodes + " / ")
                            .Append(
                                ((double)AstarPath.TotalSearchTime / 10000F).ToString("0") + ")"
                            );
#endif
                        ...
                        if (
                            Pathfinding.Util.ListPool<Pathfinding.GraphNode>.GetSize() > maxNodePool
                        )
                            maxNodePool =
                                Pathfinding.Util.ListPool<Pathfinding.GraphNode>.GetSize();
