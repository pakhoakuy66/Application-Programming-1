    ...
    public static class MovementUtilities
    {
        ...
        )
        {
            ...
            if (slowWhenNotFacingTarget && (forward.x != 0 || forward.y != 0))
            {
                ...
                angle = Mathf.Min(
                    angle,
                    (20f + 180f * (1 - slowdownFactor * slowdownFactor)) * Mathf.Deg2Rad
                );
                ...
                sin *= Mathf.Sign(
                    normalizedVelocity.x * forward.y - normalizedVelocity.y * forward.x
                );
        ...
        public static Vector2 CalculateAccelerationToReachPoint(
        ...
        )
        {
            ...
            if (targetVelocity == Vector2.zero)
            {
                ...
                while (mx - mn > 0.01f)
                {
                    ...
                    var nextA = a + q * time;
                    if (
                        a.x * a.x * ellipseSqrFactorX + a.y * a.y * ellipseSqrFactorY > 1.0f
                        || nextA.x * nextA.x * ellipseSqrFactorX
                            + nextA.y * nextA.y * ellipseSqrFactorY
                            > 1.0f
                    )
                    {
                        mn = time;
                ...
                // Boosting
                {
                    ...
                    float ellipseMagnitude =
                        finalAcceleration.x * finalAcceleration.x * ellipseSqrFactorX
            ...
            else
            {
                ...
                float targetSpeed;
                var normalizedTargetVelocity = VectorMath.Normalize(
                    targetVelocity,
                    out targetSpeed
                );
                ...
                var finalAcceleration =
                    (targetPoint.normalized * maxSpeed - currentVelocity)
