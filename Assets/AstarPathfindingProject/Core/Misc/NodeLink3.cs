...
namespace Pathfinding
{
        ...
        )
        {
            ...
            if (this.connections.Length != 2)
                throw new System.Exception(
                    "Invalid NodeLink3Node. Expected 2 connections, found "
    ...
    public class NodeLink3 : GraphModifier
    {
        protected static Dictionary<GraphNode, NodeLink3> reference =
            new Dictionary<GraphNode, NodeLink3>();
        ...
        public void InternalOnPostScan()
        {
            ...
            startNode = AstarPath.active.data.pointGraph.AddNode(
                new NodeLink3Node(AstarPath.active),
                (Int3)StartTransform.position
            );
            startNode.link = this;
            endNode = AstarPath.active.data.pointGraph.AddNode(
                new NodeLink3Node(AstarPath.active),
...
#else
            throw new System.Exception(
                "Point graphs are not included. Check your A* Optimization settings."
            );
#endif
            connectedNode1 = null;
            connectedNode2 = null;
        ...
        public void Apply(bool forceNewCheck)
        {
            ...
            uint cost = (uint)
                Mathf.RoundToInt(
                    ((Int3)(StartTransform.position - EndTransform.position)).costMagnitude
                        * costFactor
                );
            ...
            for (int a = 0; a < connectedNode1.GetVertexCount(); a++)
            {
                ...
                for (int b = 0; b < connectedNode2.GetVertexCount(); b++)
                {
                    ...
                    if (Int3.Angle((vb2 - vb1), (va2 - va1)) > (170.0 / 360.0f) * Mathf.PI * 2)
                    {
                        ...
                        t2 = System.Math.Min(
                            t2,
                            VectorMath.ClosestPointOnLineFactor(va1, va2, vb1)
                        );
                        t1 = System.Math.Max(
                            t1,
                            VectorMath.ClosestPointOnLineFactor(va1, va2, vb2)
                        );
                        ...
                        else
                        {
                            ...
                            connectedNode1.AddConnection(
                                startNode,
                                (uint)
                                    Mathf.RoundToInt(
                                        ((Int3)(clamped1 - StartTransform.position)).costMagnitude
                                            * costFactor
                                    )
                            );
                            connectedNode2.AddConnection(
                                endNode,
                                (uint)
                                    Mathf.RoundToInt(
                                        ((Int3)(clamped2 - EndTransform.position)).costMagnitude
                                            * costFactor
                                    )
                            );
        ...
        private static readonly Color GizmosColor = new Color(
            206.0f / 255.0f,
            136.0f / 255.0f,
            48.0f / 255.0f,
            0.5f
        );
        private static readonly Color GizmosColorSelected = new Color(
            235.0f / 255.0f,
            123.0f / 255.0f,
            32.0f / 255.0f,
            1.0f
            ...
            if (StartTransform != null && EndTransform != null)
            {
                ...
                if (selected)
                {
                    Vector3 cross = Vector3
                        .Cross(Vector3.up, (EndTransform.position - StartTransform.position))
                        .normalized;
                    Draw.Gizmos.Bezier(
                        StartTransform.position + cross * 0.1f,
                        EndTransform.position + cross * 0.1f,
                        col
                    );
                    Draw.Gizmos.Bezier(
                        StartTransform.position - cross * 0.1f,
