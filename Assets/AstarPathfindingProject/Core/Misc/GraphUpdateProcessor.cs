...
namespace Pathfinding
{
    ...
    class GraphUpdateProcessor
    {
...
#if !UNITY_WEBGL
        readonly System.Threading.AutoResetEvent graphUpdateAsyncEvent =
            new System.Threading.AutoResetEvent(false);
        readonly System.Threading.AutoResetEvent exitAsyncThread =
            new System.Threading.AutoResetEvent(false);
#endif
        ...
        void QueueGraphUpdatesInternal()
        {
            while (graphUpdateQueue.Count > 0)
            {
                ...
                foreach (IUpdatableGraph g in astar.data.GetUpdateableGraphs())
                {
                    ...
                    if (
                        ob.nnConstraint == null
                        || ob.nnConstraint.SuitableGraph(astar.data.GetGraphIndex(gr), gr)
                    )
                    {
        ...
        bool ProcessGraphUpdates(bool force)
        {
            ...
            Assert.AreEqual(
                graphUpdateQueueRegular.Count,
                0,
                "QueueRegular should be empty at this stage"
            );
            Assert.AreEqual(
                graphUpdateQueueAsync.Count,
                0,
                "QueueAsync should be empty at this stage"
            );
            Assert.AreEqual(
                graphUpdateQueuePost.Count,
                0,
                "QueuePost should be empty at this stage"
            );
