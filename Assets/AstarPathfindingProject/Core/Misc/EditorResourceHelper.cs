namespace Pathfinding
{
    ...
    public static class EditorResourceHelper
    {
        ...
        public static Texture2D HandlesAALineTexture
        {
            ...
            {
                if (!handlesAALineTex)
                    handlesAALineTex = Resources.Load<Texture2D>("handles_aaline");
        ...
        public static bool LocateEditorAssets()
        {
            ...
            var package = UnityEditor.PackageManager.PackageInfo.FindForAssembly(
                typeof(EditorResourceHelper).Assembly
            );
            if (package != null)
            {
                ...
                if (
                    System.IO.File.Exists(
                        package.resolvedPath + "/Editor/EditorAssets/AstarEditorSkinLight.guiskin"
                    )
                )
                {
                    return true;
                ...
                {
                    Debug.LogError(
                        "Could not find editor assets folder in package at "
                            + editorAssets
            ...
            if (
                !System.IO.File.Exists(projectPath + editorAssets + "/AstarEditorSkinLight.guiskin")
            ...
            )
            {
                ...
                while (dirQueue.Count > 0)
                {
                    ...
                    if (
                        System.IO.File.Exists(dir.FullName + "/AstarEditorSkinLight.guiskin")
