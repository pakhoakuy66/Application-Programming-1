    ...
    public class AutoRepathPolicy
    {
        ...
        public virtual bool ShouldRecalculatePath(IAstarAI ai)
        {
            ...
            else
            {
                ...
                float squaredCost =
                    (ai.destination - lastDestination).sqrMagnitude
                    / Mathf.Max(
                        (ai.position - lastDestination).sqrMagnitude,
                        ai.radius * ai.radius
                    );
                float fraction = squaredCost * (sensitivity * sensitivity);
                if (fraction > 1.0f || float.IsNaN(fraction))
                    return true;

                if (timeSinceLast >= maximumInterval * (1 - Mathf.Sqrt(fraction)))
                    return true;
                return false;
            }
        }
