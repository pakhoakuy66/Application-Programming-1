        ...
        public void JoinThreads()
        {
            if (threads != null)
            {
                for (int i = 0; i < threads.Length; i++)
                {
                    ...
                    {
                        Debug.LogError(
                            "Could not terminate pathfinding thread["
                                + i
                                + "] in 200ms, trying Thread.Abort"
                        );
                        threads[i].Abort();
                    }
                }
                ...
                while (true)
                {
                    ...
                    if (path.CompleteState == PathCompleteState.NotCalculated)
                    {
                        ...
                        System.Threading.Interlocked.Add(
                            ref AstarPath.TotalSearchTime,
                            System.DateTime.UtcNow.Ticks - startTicks
                        );
#endif
                    }
            ...
            catch (System.Exception e)
            {
...
#endif
                {
                    if (astar.logPathResults == PathLog.Heavy)
                        Debug.LogWarning(
                            "Shutting down pathfinding thread #" + pathHandler.threadID
                        );
                    return;
                }
                Debug.LogException(e);
