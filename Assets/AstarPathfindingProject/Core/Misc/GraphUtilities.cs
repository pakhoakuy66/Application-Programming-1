    ...
    public static class GraphUtilities
    {
        ...
        public static List<Vector3> GetContours(NavGraph graph)
        {
            ...
            if (graph is INavmesh)
            {
                GetContours(
                    ...
                    {
                        for (
                            int j = cycle ? vertices.Count - 1 : 0, i = 0;
                            i < vertices.Count;
                            j = i, i++
                        )
                        {
        ...
        )
        {
            ...
            var neighbourIndices =
                grid.neighbours == NumNeighbours.Six
                ...
                for (int i = 0; i < nodes.Length; i++)
                {
                    ...
                    )
                    {
                        for (int startDir = 0; startDir < neighbourIndices.Length; startDir++)
                        {
                            ...
                            var startNeighbour = startNode.GetNeighbourAlongDirection(
                                neighbourIndices[startDir]
                            );
                            if (
                                (
                                    startNeighbour == null
                                    || (nodeSet != null && !nodeSet.Contains(startNeighbour))
                            ...
                            )
                            {
                                ...
                                while (true)
                                {
                                    ...
                                    var neighbour = node.GetNeighbourAlongDirection(
                                        neighbourIndices[dir]
                                    );
                                    if (
                                        neighbour == null
                                        || (nodeSet != null && !nodeSet.Contains(neighbour))
                                    )
                                    {
                                        // Draw edge
                                        ...
                                        Vector3 graphSpacePos = new Vector3(
                                        ...
                                        graphSpacePos.x +=
                                            (neighbourXOffsets[d0] + neighbourXOffsets[d1])
                                            * offsetMultiplier;
                                        graphSpacePos.z +=
                                            (neighbourZOffsets[d0] + neighbourZOffsets[d1])
                                            * offsetMultiplier;
                                        graphSpacePos.y = grid.transform
                                            .InverseTransform((Vector3)node.position)
                                            .y;
                                        ...
                                        if (trace.Count >= 2)
                                        {
                                            ...
                                            if (
                                                (
                                                    (
                                                        Mathf.Abs(v1d.x) > 0.01f
                                                        || Mathf.Abs(v2d.x) > 0.01f
                                                    )
                                                    && (
                                                        Mathf.Abs(v1d.z) > 0.01f
                                                        || Mathf.Abs(v2d.z) > 0.01f
                                                    )
                                                )
                                                || (
                                    ...
                                    else
                                    {
                                        ...
                                        node = neighbour;
                                        dir =
                                            (dir + neighbourIndices.Length / 2 + 1)
