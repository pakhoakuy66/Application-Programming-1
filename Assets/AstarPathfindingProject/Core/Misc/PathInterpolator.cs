...
namespace Pathfinding.Util
{
    ...
    public class PathInterpolator
    {
        ...
        {
            if (!valid)
                throw new System.Exception("PathInterpolator is not valid");
            buffer.Add(position);
        ...
        public void SetPath(List<Vector3> path)
        {
            ...
            if (path.Count < 2)
                throw new System.ArgumentException("Path must have a length of at least 2");
        ...
        public void MoveToSegment(int index, float fractionAlongSegment)
        {
            ...
            while (segmentIndex < index)
                NextSegment();
            distance =
                distanceToSegmentStart + Mathf.Clamp01(fractionAlongSegment) * currentSegmentLength;
        }
        ...
        public void MoveToLocallyClosestPoint(
            Vector3 point,
        ...
        )
        {
            ...
            while (
                allowForwards
                && segmentIndex < path.Count - 2
                && (path[segmentIndex + 1] - point).sqrMagnitude
            ...
            if (segmentIndex > 0)
            {
                factor1 = VectorMath.ClosestPointOnLineFactor(
                    path[segmentIndex - 1],
                    path[segmentIndex],
                    point
                );
                d1 = (
                    Vector3.Lerp(path[segmentIndex - 1], path[segmentIndex], factor1) - point
                ).sqrMagnitude;
            }

            if (segmentIndex < path.Count - 1)
            {
                factor2 = VectorMath.ClosestPointOnLineFactor(
                    path[segmentIndex],
                    path[segmentIndex + 1],
                    point
                );
                d2 = (
                    Vector3.Lerp(path[segmentIndex], path[segmentIndex + 1], factor2) - point
                ).sqrMagnitude;
            }
        ...
        )
        {
            ...
            while (
                segmentIndex < path.Count - 2
                && VectorMath.ClosestPointOnLineFactor(
                    path[segmentIndex],
                    path[segmentIndex + 1],
                    circleCenter3D
                ) > 1
            ...
            while (
                segmentIndex < path.Count - 2
                && (transform.ToPlane(path[segmentIndex + 1]) - circleCenter).sqrMagnitude
                    <= radius * radius
            )
            {
                NextSegment();
            ...
            var factor = VectorMath.LineCircleIntersectionFactor(
                circleCenter,
                transform.ToPlane(path[segmentIndex]),
                transform.ToPlane(path[segmentIndex + 1]),
                radius
            );
