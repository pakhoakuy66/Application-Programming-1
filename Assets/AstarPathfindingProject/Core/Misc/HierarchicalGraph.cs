    ...
    public class HierarchicalGraph
    {
        ...
        public HierarchicalGraph()
        {
            ...
            connectionCallback = (GraphNode neighbour) =>
            {
                ...
                {
                    if (
                        currentChildren.Count < MaxChildrenPerNode && neighbour.Walkable /* && (((GridNode)currentChildren[0]).XCoordinateInGrid/Tiling == ((GridNode)neighbour).XCoordinateInGrid/Tiling) && (((GridNode)currentChildren[0]).ZCoordinateInGrid/Tiling == ((GridNode)neighbour).ZCoordinateInGrid/Tiling)*/
                    )
                    {
                    ...
                    hIndex != currentHierarchicalNodeIndex && !currentConnections.Contains(hIndex)
                )
                {
        ...
        internal void OnCreatedNode(GraphNode node)
        {
            ...
            {
                var newDirty = new GraphNode[
                    System.Math.Max(node.NodeIndex + 1, dirtyNodes.Length * 2)
                ];
        ...
        void RemoveHierarchicalNode(int hierarchicalNode, bool removeAdjacentSmallNodes)
        {
            ...
            for (int i = 0; i < conns.Count; i++)
            {
                ...
                if (dirty[adjacentHierarchicalNode] != 0)
                    continue;

                if (
                    removeAdjacentSmallNodes
                    && children[adjacentHierarchicalNode].Count < MinChildrenPerNode
                )
                {
