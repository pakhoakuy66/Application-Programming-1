    ...
    public abstract class Path : IPathInternals
    {
        ...
        internal uint CalculateHScore(GraphNode node)
        {
            ...
            switch (heuristic)
            {
                ...
                case Heuristic.Manhattan:
                    ...
                    h = (uint)(
                        (
                            System.Math.Abs(hTarget.x - p2.x)
                            + System.Math.Abs(hTarget.y - p2.y)
                            + System.Math.Abs(hTarget.z - p2.z)
                        ) * heuristicScale
                    );
                    return h;
        ...
        private void ErrorCheck()
        {
            ...
            if (((IPathInternals)this).Pooled)
                FailWithError(
                    "The path is currently in a path pool. Are you sending the path for calculation twice?"
                );
            if (pathHandler == null)
        ...
        protected virtual void OnEnterPool()
        {
            ...
            if (path != null)
                Pathfinding.Util.ListPool<GraphNode>.Release(ref path);
            ...
            for (int i = 0; i < claimed.Count; i++)
            {
                ...
                if (System.Object.ReferenceEquals(claimed[i], o))
                    throw new System.ArgumentException(
                        "You have already claimed the path with that object ("
                            + o
        ...
        public void Release(System.Object o, bool silent = false)
        {
            ...
            if (claimed.Count == 0)
            {
                throw new System.ArgumentException(
                    "You are releasing a path which is not claimed at all (most likely it has been pooled already). "
                        + "Are you releasing the path with the same object ("
                        + o
                        + ") twice?"
            ...
            throw new System.ArgumentException(
                "You are releasing a path which has not been claimed with this object ("
                    + o
                    + "). "
