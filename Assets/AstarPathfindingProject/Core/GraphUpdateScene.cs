...
namespace Pathfinding
{
    ...
    public class GraphUpdateScene : GraphModifier
    {
        ...
        public Bounds GetBounds()
        {
            ...
            {
                return GraphUpdateShape.GetBounds(
                    convex ? convexPoints : points,
                    legacyMode && legacyUseWorldSpace
                        ? Matrix4x4.identity
        ...
        void OnDrawGizmos(bool selected)
        {
            ...
            if (selected || !convex)
            {
                ...
                {
                    Gizmos.DrawLine(
                        matrix.MultiplyPoint3x4(points[i]),
            ...
            if (convex)
            {
                ...
                {
                    Gizmos.DrawLine(
                        matrix.MultiplyPoint3x4(convexPoints[i]),
            ...
            if (selected && pts != null && pts.Length > 0)
            {
                ...
                for (int i = 0; i < pts.Length; i++)
                {
                    ...
                    var p1n = matrix.MultiplyPoint3x4(
                        pts[next] + Vector3.up * (miny - pts[next].y)
                    );
                    var p2n = matrix.MultiplyPoint3x4(
                        pts[next] + Vector3.up * (maxy - pts[next].y)
                    );
