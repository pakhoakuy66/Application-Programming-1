...
namespace Pathfinding.Serialization
{
    ...
    public class TinyJsonSerializer
    {
        ...
        Dictionary<Type, Action<System.Object>> serializers =
            new Dictionary<Type, Action<object>>();

        static readonly System.Globalization.CultureInfo invariantCulture = System
            .Globalization
            .CultureInfo
            .InvariantCulture;
        ...
        TinyJsonSerializer()
        {
            ...
            serializers[typeof(Version)] =
                serializers[typeof(uint)] =
                serializers[typeof(int)] =
                    v => output.Append(v.ToString());
            serializers[typeof(string)] = v =>
                output.AppendFormat("\"{0}\"", v.ToString().Replace("\"", "\\\""));
            serializers[typeof(Vector2)] = v =>
                output.AppendFormat(
                    "{{ \"x\": {0}, \"y\": {1} }}",
                    ((Vector2)v).x.ToString("R", invariantCulture),
                    ((Vector2)v).y.ToString("R", invariantCulture)
                );
            serializers[typeof(Vector3)] = v =>
                output.AppendFormat(
                    "{{ \"x\": {0}, \"y\": {1}, \"z\": {2} }}",
                    ((Vector3)v).x.ToString("R", invariantCulture),
                    ((Vector3)v).y.ToString("R", invariantCulture),
                    ((Vector3)v).z.ToString("R", invariantCulture)
                );
            serializers[typeof(Pathfinding.Util.Guid)] = v =>
        ...
        void Serialize(System.Object obj)
        {
            ...
            {
#if NETFX_CORE
                var optIn = typeInfo.CustomAttributes.Any(
                    attr => attr.GetType() == typeof(JsonOptInAttribute)
                );
#else
                var optIn =
                    typeInfo.GetCustomAttributes(typeof(JsonOptInAttribute), true).Length > 0;
#endif
                output.Append("{");
                bool earlier = false;
                ...
                while (true)
                {
                    ...
                    foreach (var field in fields)
                    {
                        if (field.DeclaringType != type)
                            continue;
                        if (
                            (!optIn && field.IsPublic)
                            ||
#if NETFX_CORE
                            field.CustomAttributes.Any(
                                attr => attr.GetType() == typeof(JsonMemberAttribute)
                            )
#else
    ...
    public class TinyJsonDeserializer
    {
        ...
        static readonly System.Globalization.NumberFormatInfo numberFormat = System
            .Globalization
            .NumberFormatInfo
            .InvariantInfo;
        ...
        System.Object Deserialize(Type tp, System.Object populate = null)
        {
            ...
            else if (tpInfo.IsArray)
            {
                ...
                Type.Equals(tp, typeof(Mesh))
                || Type.Equals(tp, typeof(Texture2D))
                || Type.Equals(tp, typeof(Transform))
                || Type.Equals(tp, typeof(GameObject))
            )
            {
        ...
        UnityEngine.Object DeserializeUnityObjectInner()
        {
            ...
            var type = WindowsStoreCompatibility
                .GetTypeInfo(typeof(AstarPath))
                .Assembly.GetType(typename);
            type =
                type
                ?? WindowsStoreCompatibility
                    .GetTypeInfo(typeof(Transform))
                    .Assembly.GetType(typename);

            if (Type.Equals(type, null))
            {
                Debug.LogError(
                    "Could not find type '" + typename + "'. Cannot deserialize Unity reference"
                );
                return null;
            }
            ...
            if ((char)reader.Peek() == '"')
            {
                ...
                {
                    foreach (
                        var helper in contextRoot.GetComponentsInChildren<UnityReferenceHelper>(
                            true
                        )
                    )
                    {
                ...
                foreach (
                    var helper in UnityEngine.Object.FindObjectsOfType<UnityReferenceHelper>(true)
                )
#else
