...
namespace Pathfinding
{
    ...
    public abstract class GraphNode
    {
        ...
        internal bool TemporaryFlag1
        {
            ...
            set
            {
                nodeIndex = (nodeIndex & ~TemporaryFlag1Mask) | (value ? TemporaryFlag1Mask : 0);
            }
        }
        ...
        internal bool TemporaryFlag2
        {
            ...
            set
            {
                nodeIndex = (nodeIndex & ~TemporaryFlag2Mask) | (value ? TemporaryFlag2Mask : 0);
            }
        }
        ...
        public const uint MaxHierarchicalNodeIndex =
            HierarchicalIndexMask >> FlagsHierarchicalIndexOffset;
        ...
        internal int HierarchicalNodeIndex
        {
            ...
            {
                flags =
                    (flags & ~HierarchicalIndexMask)
                    | (uint)(value << FlagsHierarchicalIndexOffset);
            }
        }
        ...
        internal bool IsHierarchicalNodeDirty
        {
            ...
            {
                flags =
                    flags & ~HierarchicalDirtyMask | (value ? 1U : 0U) << HierarchicalDirtyOffset;
            }
        }
        ...
        public uint Area
        {
            get
            {
                return AstarPath.active.hierarchicalGraph.GetConnectedComponent(
        ...
        public virtual void DeserializeNode(GraphSerializationContext ctx)
        {
            ...
            Flags =
                (ctx.reader.ReadUInt32() & ~(HierarchicalIndexMask | HierarchicalDirtyMask))
            ...
            if (connections != null)
            {
                for (int i = 0; i < connections.Length; i++)
                {
                    if (connections[i].node == node)
                    {
                        ...
                        connections[i].shapeEdge =
                            shapeEdge >= 0 ? (byte)shapeEdge : connections[i].shapeEdge;
                        return;
                    }
                }
            }
            ...
            else
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    connections[i] = new Connection(
                        ...
                        ctx.meta.version < AstarSerializer.V4_1_0
                            ? (byte)0xFF
                            : ctx.reader.ReadByte()
                    );
                }
