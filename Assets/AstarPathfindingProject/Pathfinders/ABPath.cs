    ...
    public class ABPath : Path
    {
        ...
        public override uint GetConnectionSpecialCost(GraphNode a, GraphNode b, uint currentCost)
        {
            if (startNode != null && endNode != null)
            {
                ...
                {
                    return (uint)(
                        (startIntPoint - (b == endNode ? hTarget : b.position)).costMagnitude
                ...
                {
                    return (uint)(
                        (startIntPoint - (a == endNode ? hTarget : a.position)).costMagnitude
                        * (currentCost * 1.0 / (a.position - b.position).costMagnitude)
                    );
                }
                if (a == endNode)
                {
                    return (uint)(
                        (hTarget - b.position).costMagnitude
                        * (currentCost * 1.0 / (a.position - b.position).costMagnitude)
                    );
                }
                if (b == endNode)
                {
                    return (uint)(
                        (hTarget - a.position).costMagnitude
                        * (currentCost * 1.0 / (a.position - b.position).costMagnitude)
                    );
                }
            }
            else
            {
                ...
                {
                    return (uint)(
                        (startIntPoint - b.position).costMagnitude
                        * (currentCost * 1.0 / (a.position - b.position).costMagnitude)
                    );
                }
                if (b == startNode)
                {
                    return (uint)(
                        (startIntPoint - a.position).costMagnitude
                        * (currentCost * 1.0 / (a.position - b.position).costMagnitude)
                    );
                }
            }
        ...
        protected virtual bool EndPointGridGraphSpecialCase(GraphNode closestWalkableEndNode)
        {
            ...
            if (gridNode != null)
            {
                ...
                if (
                    gridNode != gridNode2
                    && gridNode2 != null
                    && gridNode.GraphIndex == gridNode2.GraphIndex
                )
                {
                    ...
                    switch (gridGraph.neighbours)
                    {
                        ...
                        case NumNeighbours.Six:
                            ...
                            for (int i = 0; i < 6; i++)
                            {
                                var nx =
                                    x2
                                    + gridGraph.neighbourXOffsets[
                                        GridGraph.hexagonNeighbourIndices[i]
                                    ];
                                var nz =
                                    z2
                                    + gridGraph.neighbourZOffsets[
                                        GridGraph.hexagonNeighbourIndices[i]
                                    ];
                                if (x1 == nx && z1 == nz)
                                {
            ...
            else
            {
                ...
                if (gridNode == null)
                {
                    throw new System.Exception(
                        "Some path is not cleaning up the flag1 field. This is a bug."
                    );
                }
        ...
        protected override void CalculateStep(long targetTick)
        {
            ...
            while (CompleteState == PathCompleteState.NotCalculated)
            {
                ...
                if (counter > 500)
                {
                    ...
                    {
                        throw new System.Exception(
                            "Probable infinite loop. Over 1,000,000 nodes searched"
                        );
                    }
                }
        ...
        public Vector3 GetMovementVector(Vector3 point)
        {
            ...
            {
                Vector3 closest = VectorMath.ClosestPointOnSegment(
                    vectorPath[i],
                    vectorPath[i + 1],
                    point
                );
