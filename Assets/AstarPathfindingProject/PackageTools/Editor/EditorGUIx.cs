...
namespace Pathfinding
{
    ...
    public static class EditorGUILayoutx
    {
        ...
        public static int TagField(string label, int value, System.Action editCallback)
        {
            ...
            if (
                tagNamesAndEditTagsButton == null
                || EditorApplication.timeSinceStartup - timeLastUpdatedTagNames > 1
            )
            {
        ...
        {
            if (tagMask == null)
                throw new System.ArgumentNullException("tagMask");
            if (EditorGUILayout.Foldout(foldout, label))
            {
                ...
                if (GUILayout.Button("Add Tag"))
                    tagMask.Add("Untagged");

                EditorGUI.BeginDisabledGroup(tagMask.Count == 0);
                if (GUILayout.Button("Remove Last"))
                    tagMask.RemoveAt(tagMask.Count - 1);
        ...
        public static LayerMask LayerMaskField(string label, LayerMask selected)
        {
            if (
                ...
                && System.DateTime.UtcNow.Ticks - lastUpdateTick > 10000000L
            )
            {
                layerNames.Clear();
