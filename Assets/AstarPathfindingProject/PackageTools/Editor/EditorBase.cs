    ...
    public class EditorBase : Editor
    {
        ...
        public sealed override void OnInspectorGUI()
        {
            ...
            if (targets.Length == 1 && (target as MonoBehaviour).enabled)
            {
                var attr = target
                    .GetType()
                    .GetCustomAttributes(typeof(UniqueComponentAttribute), true);
                for (int i = 0; i < attr.Length; i++)
                {
                    ...
                    {
                        if (!other.enabled || other == target)
                            continue;
                        if (
                            other
                                .GetType()
                                .GetCustomAttributes(typeof(UniqueComponentAttribute), true)
                                .Select(c => (c as UniqueComponentAttribute).tag == tag)
                                .Any()
                        )
                        {
        ...
        void CaptureContextClick(string propertyPath)
        {
            ...
            if (url != null && getDocumentationURL != null)
            {
                ...
                menu.AddItem(
                    showInDocContent,
                    false,
                    () => Application.OpenURL(getDocumentationURL() + url)
                );
                menu.ShowAsContext();
            }
        }
        ...
        protected void Popup(string propertyPath, GUIContent[] options, string label = null)
        {
            ...
            int newVal = EditorGUILayout.Popup(
                content,
            ...
            if (
                contextClick && GUILayoutUtility.GetLastRect().Contains(Event.current.mousePosition)
            )
                CaptureContextClick(propertyPath);
        }
