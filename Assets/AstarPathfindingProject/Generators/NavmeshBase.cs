    ...
    public abstract class NavmeshBase : NavGraph, INavmesh, INavmeshHolder, ITransformedGraph
    {
        ...
        public Bounds GetTileBoundsInGraphSpace(int x, int z, int width = 1, int depth = 1)
        {
            ...
            b.SetMinMax(
                ...
                new Vector3(
                    (x + width) * TileWorldSizeX,
                    forcedBoundsSize.y,
                    (z + depth) * TileWorldSizeZ
                )
            );
            return b;
        }
        ...
        public IntRect GetTouchingTiles(Bounds bounds, float margin = 0)
        {
            ...
            var r = new IntRect(
                Mathf.FloorToInt((bounds.min.x - margin) / TileWorldSizeX),
        ...
        public IntRect GetTouchingTilesInGraphSpace(Rect rect)
        {
            // Calculate world bounds of all affected tiles
            var r = new IntRect(
                Mathf.FloorToInt(rect.xMin / TileWorldSizeX),
                Mathf.FloorToInt(rect.yMin / TileWorldSizeZ),
                Mathf.FloorToInt(rect.xMax / TileWorldSizeX),
        ...
        public IntRect GetTouchingTilesRound(Bounds bounds)
        {
            ...
            //Calculate world bounds of all affected tiles
            var r = new IntRect(
                Mathf.RoundToInt(bounds.min.x / TileWorldSizeX),
                Mathf.RoundToInt(bounds.min.z / TileWorldSizeZ),
                Mathf.RoundToInt(bounds.max.x / TileWorldSizeX) - 1,
        ...
        static readonly NNConstraint NNConstraintDistanceXZ = new NNConstraint
        {
            distanceXZ = true
        };

        public override NNInfoInternal GetNearest(
            Vector3 position,
            NNConstraint constraint,
            GraphNode hint
        )
        {
            return GetNearestForce(
                position,
        ...
        public override NNInfoInternal GetNearestForce(Vector3 position, NNConstraint constraint)
        {
            ...
            for (int w = 0; w < wmax; w++)
            {
                ...
                if (bestDistance < (w - 2) * Math.Max(TileWorldSizeX, TileWorldSizeX))
                    break;
                    ...
                    do
                    {
                        ...
                        if (x >= 0 && x < tileXCount)
                        {
                            ...
                            if (tile != null)
                            {
                                if (xzSearch)
                                {
                                    best = tile.bbTree.QueryClosestXZ(
                                        position,
                                        constraint,
                                        ref bestDistance,
                                        best
        ...
        public GraphNode PointOnNavmesh(Vector3 position, NNConstraint constraint)
        {
            ...
            if (
                tileCoords.x < 0
                || tileCoords.y < 0
                || tileCoords.x >= tileXCount
                || tileCoords.y >= tileZCount
            )
                return null;
                ...
                for (int a = 0; a < av; a++)
                {
                    ...
                    if (nodeRefs.TryGetValue(new Int2(second, first), out connNode))
                    {
                        ...
                        for (int b = 0; b < bv; b++)
                        {
                            ...
                            if (
                                other.GetVertexIndex(b) == second
                                && other.GetVertexIndex((b + 1) % bv) == first
                            )
                            {
                                connections.Add(
                                    new Connection(
        ...
        protected void ConnectTiles(NavmeshTile tile1, NavmeshTile tile2)
        {
            ...
            {
                throw new System.ArgumentException(
                    "Tiles are not adjacent (neither x or z coordinates match)"
                );
            }
            ...
            int midpoint = (int)
                Math.Round((Math.Max(t1coord, t2coord) * tileWorldSize) * Int3.Precision);
            ...
            for (int j = 0; j < nodes2.Length; j++)
            {
                ...
                for (int b = 0; b < bVertexCount; b++)
                {
                    ...
                    if (
                        Math.Abs(bVertex1[coord] - midpoint) < 2
            ...
            for (int i = 0; i < nodes1.Length; i++)
            {
                ...
                for (int a = 0; a < aVertexCount; a++)
                {
                    ...
                    if (
                        Math.Abs(aVertex1[coord] - midpoint) < 2
                        && Math.Abs(aVertex2[coord] - midpoint) < 2
                    )
                    {
                            ...
                            for (int b = 0; b < bVertexCount; b++)
                            {
                                ...
                                )
                                {
                                    ...
                                    if (maxalt > minalt2 && minalt < maxalt2)
                                    {
                                        ...
                                        if (
                                            (aVertex1 == bVertex1 && aVertex2 == bVertex2)
                                            || (aVertex1 == bVertex2 && aVertex2 == bVertex1)
                                            || VectorMath.SqrDistanceSegmentSegment(
                                                (Vector3)aVertex1,
                                                (Vector3)aVertex2,
                                                (Vector3)bVertex1,
                                                (Vector3)bVertex2
                                        ...
                                        {
                                            uint cost = (uint)
                                                (nodeA.position - nodeB.position).costMagnitude;
        ...
        public void StartBatchTileUpdate()
        {
            if (batchTileUpdate)
                throw new System.InvalidOperationException(
                    "Calling StartBatchLoad when batching is already enabled"
                );
        ...
        public void EndBatchTileUpdate()
        {
            ...
            for (int i = 0; i < batchUpdatedTiles.Count; i++)
            {
                int x = batchUpdatedTiles[i] % tileXCount,
                    z = batchUpdatedTiles[i] / tileXCount;
                if (x > 0)
                    TryConnect(batchUpdatedTiles[i], batchUpdatedTiles[i] - 1);
                if (x < tileXCount - 1)
                    TryConnect(batchUpdatedTiles[i], batchUpdatedTiles[i] + 1);
                if (z > 0)
                    TryConnect(batchUpdatedTiles[i], batchUpdatedTiles[i] - tileXCount);
                if (z < tileZCount - 1)
                    TryConnect(batchUpdatedTiles[i], batchUpdatedTiles[i] + tileXCount);
            }

            for (int i = 0; i < batchUpdatedTiles.Count; i++)
                tiles[batchUpdatedTiles[i]].flag = false;
        ...
        )
        {
            ...
            {
                recycling[
                    verts[tris[i + 0]].GetHashCode()
                        + verts[tris[i + 1]].GetHashCode()
                        + verts[tris[i + 2]].GetHashCode()
                ] = j;
            }
            ...
            for (int i = 0; i < nodes.Length; i++)
            {
                ...
                if (recycling.TryGetValue(hash, out newNodeIndex))
                {
                    ...
                    if (
                        verts[tris[3 * newNodeIndex + 0]] == v0
                        && verts[tris[3 * newNodeIndex + 1]] == v1
        ...
        public void ReplaceTile(int x, int z, Int3[] verts, int[] tris)
        {
            ...
            if (tris.Length % 3 != 0)
                throw new System.ArgumentException(
                    "Triangle array's length must be a multiple of 3 (tris)"
                );
            ...
            if (
                !Mathf.Approximately(
                    x * TileWorldSizeX * Int3.FloatPrecision,
            ...
            if (
                !Mathf.Approximately(
                    z * TileWorldSizeZ * Int3.FloatPrecision,
            ...
            CreateNodes(
                ...
                tile.tris,
                x + z * tileXCount,
                (uint)active.data.GetGraphIndex(this)
            );
        ...
        )
        {
            if (buffer == null || buffer.Length < tris.Length / 3)
                throw new System.ArgumentException(
                    "buffer must be non null and at least as large as tris.Length/3"
                );
            ...
            for (int i = 0; i < buffer.Length; i++)
            {
                ...
                if (node == null)
                    node = buffer[i] = new TriangleMeshNode(active);
                ...
                if (
                    RecalculateNormals
                    && !VectorMath.IsClockwiseXZ(
                        node.GetVertexInGraphSpace(0),
                        node.GetVertexInGraphSpace(1),
                        node.GetVertexInGraphSpace(2)
                    )
                )
                {
        ...
        static void CreateNavmeshOutlineVisualization(
            NavmeshTile[] tiles,
            int startTile,
            int endTile,
            GraphGizmoHelper helper
            ...
            for (int i = startTile; i < endTile; i++)
            {
                ...
                for (int j = 0; j < tile.nodes.Length; j++)
                {
                    ...
                    for (int c = 0; c < node.connections.Length; c++)
                    {
                        ...
                        if (other != null && other.GraphIndex == node.GraphIndex)
                        {
                            for (int v = 0; v < 3; v++)
                            {
                                ...
                                {
                                    if (
                                        node.GetVertexIndex(v) == other.GetVertexIndex((v2 + 1) % 3)
                                        && node.GetVertexIndex((v + 1) % 3)
                                            == other.GetVertexIndex(v2)
                                    )
                                    {
                    ...
                    for (int v = 0; v < 3; v++)
                    {
                        if (!sharedEdges[v])
                        {
                            helper.builder.DrawLine(
                                (Vector3)node.GetVertex(v),
                                (Vector3)node.GetVertex((v + 1) % 3),
                                color
                            );
                        }
                    }
        ...
        protected override void DeserializeExtraInfo(GraphSerializationContext ctx)
        {
            ...
            for (int z = 0; z < tileZCount; z++)
            {
                for (int x = 0; x < tileXCount; x++)
                {
                    ...
                    if (tx < 0)
                        throw new System.Exception("Invalid tile coordinates (x < 0)");

                    int tz = reader.ReadInt32();
                    if (tz < 0)
                        throw new System.Exception("Invalid tile coordinates (z < 0)");
                    ...
                    if (trisCount % 3 != 0)
                        throw new System.Exception(
                            "Corrupt data. Triangle indices count must be divisable by 3. Read "
                                + trisCount
                        );

                    tile.tris = new int[trisCount];
                    for (int i = 0; i < tile.tris.Length; i++)
                        tile.tris[i] = reader.ReadInt32();
        ...
        protected override void PostDeserialization(GraphSerializationContext ctx)
        {
            ...
            {
                Dictionary<TriangleMeshNode, Connection[]> conns = tiles
                ...
                GetNodes(node =>
                {
                    ...
                    foreach (
                        var conn in conns[triNode]
                            .Where(conn => !triNode.ContainsConnection(conn.node))
                            .ToList()
                    )
                    {
