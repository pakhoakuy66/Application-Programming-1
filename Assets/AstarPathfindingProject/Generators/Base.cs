...
namespace Pathfinding
{
    ...
    public abstract class NavGraph : IGraphInternals
    {
        ...
        {
            GetNodes(
                node => node.position = ((Int3)deltaMatrix.MultiplyPoint((Vector3)node.position))
            );
        }
        ...
        /// <param name="constraint">Can for example tell the function to try to return a walkable node. If you do not get a good node back, consider calling GetNearestForce.</param>
        public virtual NNInfoInternal GetNearest(
            Vector3 position,
    ...
    public class GraphCollision
    {
        ...
        public Vector3 CheckHeight(Vector3 position, out RaycastHit hit, out bool walkable)
        {
            ...
            if (thickRaycast)
            {
                ...
                {
                    return VectorMath.ClosestPointOnLine(
                        ray.origin,
        ...
        public RaycastHit[] CheckHeightAll(Vector3 position, out int numHits)
        {
            if (!heightCheck || use2D)
            {
                hitBuffer[0] = new RaycastHit { point = position, distance = 0, };
                numHits = 1;
                return hitBuffer;
            }
    ...
    public enum RayDirection
    {
        ...
        Both
        /// <summary>< Casts two rays in both directions</summary>
    }
}
