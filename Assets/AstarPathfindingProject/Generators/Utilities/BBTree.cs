    ...
    public class BBTree : IAstarPooledObject
    {
            ...
            get
            {
                ...
                else
                {
                    ...
                    return Rect.MinMaxRect(
                        rect.xmin * Int3.PrecisionFactor,
                        rect.ymin * Int3.PrecisionFactor,
                        rect.xmax * Int3.PrecisionFactor,
                        rect.ymax * Int3.PrecisionFactor
                    );
                }
            }
        }
        ...
        )
        {
            ...
            if (to - from <= MaximumLeafSize)
            {
                ...
                {
                    nodeLookup[nodeOffset + i] =
                        i < to - from ? nodes[permutation[from + i]] : null;
                }
                return box;
            }
            ...
            tree[box].left = RebuildFromInternal(
                nodes,
                permutation,
                nodeBounds,
                from,
            ...
            tree[box].right = RebuildFromInternal(
                nodes,
                permutation,
                nodeBounds,
                splitIndex,
        ...
        {
            Debug.DrawLine(
                (Vector3)node.GetVertex(1) + Vector3.up * yoffset,
                (Vector3)node.GetVertex(2) + Vector3.up * yoffset,
                color
            );
            Debug.DrawLine(
                (Vector3)node.GetVertex(0) + Vector3.up * yoffset,
                (Vector3)node.GetVertex(1) + Vector3.up * yoffset,
                color
            );
            Debug.DrawLine(
        ...
        public NNInfoInternal QueryClosestXZ(
            Vector3 p,
            ...
            if (box.IsLeaf)
            {
                ...
                for (int i = 0; i < MaximumLeafSize && nodes[box.nodeOffset + i] != null; i++)
                {
                    ...
                    if (constraint == null || constraint.Suitable(node))
                    {
                        ...
                        // XZ squared distance
                        float dist =
                            (closest.x - p.x) * (closest.x - p.x)
                            + (closest.z - p.z) * (closest.z - p.z);
                        ...
                        if (
                            ...
                            || dist < closestSqrDist - fuzziness
                            || (
                                dist <= closestSqrDist + fuzziness
                                && Mathf.Abs(closest.y - p.y)
        ...
        public NNInfoInternal QueryClosest(
            Vector3 p,
            NNConstraint constraint,
            ref float distance,
            NNInfoInternal previous
