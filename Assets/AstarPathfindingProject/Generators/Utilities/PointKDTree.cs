...
namespace Pathfinding
{
    ...
    public class PointKDTree
    {
        ...
        int Size(int index)
        {
            return tree[index].data != null
                ? tree[index].count
                : Size(2 * index) + Size(2 * index + 1);
        }
        ...
        void Build(int index, List<GraphNode> nodes, int start, int end)
        {
            ...
            else
            {
                ...
                for (int i = start; i < end; i++)
                {
                    ...
                    mn = new Int3(
                        System.Math.Min(mn.x, p.x),
                        System.Math.Min(mn.y, p.y),
                        System.Math.Min(mn.z, p.z)
                    );
                    mx = new Int3(
                        System.Math.Max(mx.x, p.x),
                        System.Math.Max(mx.y, p.y),
                        System.Math.Max(mx.z, p.z)
                    );
                }
                Int3 diff = mx - mn;
                ...
                int mid = (start + end) / 2;
                tree[index].split =
                    (nodes[mid - 1].position[axis] + nodes[mid].position[axis] + 1) / 2;
        ...
        void Add(GraphNode point, int index, int depth = 0)
        {
            ...
            {
                index =
                    2 * index + (point.position[tree[index].splitAxis] < tree[index].split ? 0 : 1);
                depth++;
            }
            ...
            if (tree[index].count >= LeafArraySize)
            {
                ...
                while (
                    depth - levelsUp > 0
                    && Size(index >> levelsUp) > MaxAllowedSize(numNodes, depth - levelsUp)
                )
                {
                    levelsUp++;
                }
        ...
        /// <summary>Closest node to the point which satisfies the constraint</summary>
        public GraphNode GetNearestConnection(
            Int3 point,
            NNConstraint constraint,
            long maximumSqrConnectionLength
        )
        {
            GraphNode best = null;
            long bestSqrDist = long.MaxValue;
        ...
        )
        {
            ...
            if (data != null)
            {
                ...
                for (int i = tree[index].count - 1; i >= 0; i--)
                {
                    ...
                    if (
                        dist - distanceThresholdOffset < bestSqrDist
                    ...
                    )
                    {
                        ...
                        if (conns != null)
                        {
                            ...
                            for (int j = 0; j < conns.Length; j++)
                            {
                                ...
                                float sqrConnectionDistance = VectorMath.SqrDistancePointSegment(
                                    nodePos,
                                    connectionMidpoint,
                                    pointv3
                                );
                                // Convert to Int3 space
                                long sqrConnectionDistanceInt = (long)(
                                    sqrConnectionDistance
                                    * Int3.FloatPrecision
                                    * Int3.FloatPrecision
                                );
