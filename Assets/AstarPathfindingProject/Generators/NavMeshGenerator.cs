    ...
    public class NavMeshGraph : NavmeshBase, IUpdatableGraph
    {
        ...
        public override GraphTransform CalculateTransform()
        {
            return new GraphTransform(
                Matrix4x4.TRS(offset, Quaternion.Euler(rotation), Vector3.one)
                    * Matrix4x4.TRS(
                        sourceMesh != null
                            ? sourceMesh.bounds.min * scale
                            : cachedSourceMeshBoundsMin * scale,
                        Quaternion.identity,
                        Vector3.one
                    )
            );
        ...
        public static void UpdateArea(GraphUpdateObject o, INavmeshHolder graph)
        {
            ...
            graph.GetNodes(_node =>
            {
                ...
                if (
                    inside
                    || node.ContainsPointInGraphSpace(a)
                    || node.ContainsPointInGraphSpace(b)
                    || node.ContainsPointInGraphSpace(c)
                    || node.ContainsPointInGraphSpace(d)
                )
                {
                    inside = true;
                }
        ...
        [System.Obsolete(
            "Set the mesh to ObjImporter.ImportFile(...) and scan the graph the normal way instead"
        )]
        ...
        protected override IEnumerable<Progress> ScanInternal()
        {
            ...
            Polygon.CompressMesh(
                intVertices,
                new List<int>(sourceMesh.triangles),
                out compressedVertices,
