...
namespace Pathfinding
{
    ...
    public class PointGraph : NavGraph
    {
            ...
            if (conns != null)
            {
                for (int i = 0; i < conns.Length; i++)
                {
                    var connectionMidpoint =
                        ((UnityEngine.Vector3)conns[i].node.position + nodePos) * 0.5f;
                    var closestPoint = VectorMath.ClosestPointOnSegment(
                        nodePos,
                        connectionMidpoint,
                        position
                    );
            ...
            where T : PointNode
        {
            ...
            {
                var newNodes = new PointNode[
                    nodes != null ? System.Math.Max(nodes.Length + 4, nodes.Length * 2) : 4
                ];
                if (nodes != null)
        ...
        protected override IEnumerable<Progress> ScanInternal()
        {
            ...
            if (root == null)
            {
                ...
                GameObject[] gos =
                    searchTag != null ? GameObject.FindGameObjectsWithTag(searchTag) : null;
            ...
            foreach (var progress in ConnectNodesAsync())
                yield return progress.MapTo(0.15f, 0.95f);
        }
                ...
                else
                {
                    maxSquaredRange =
                        (long)(
                            Mathf.Max(
                                limits.x,
                                Mathf.Max(limits.y, Mathf.Max(limits.z, maxDistance))
                            ) * Int3.Precision
                        ) + 1;
                    ...
                    if (use2DPhysics)
                    {
                        ...
                        else
                        {
                            return !Physics2D.Linecast(
                                    (Vector2)(Vector3)a.position,
                                    (Vector2)(Vector3)b.position,
                                    mask
                                )
                                && !Physics2D.Linecast(
                                    (Vector2)(Vector3)b.position,
            ...
            else if (!string.IsNullOrEmpty(searchTag))
            {
                ...
                for (int i = 0; i < gos.Length; i++)
                {
                    Gizmos.DrawCube(
                        ...
                        Vector3.one
                            * UnityEditor.HandleUtility.GetHandleSize(gos[i].transform.position)
                            * 0.1F
                    );
                }
            }
