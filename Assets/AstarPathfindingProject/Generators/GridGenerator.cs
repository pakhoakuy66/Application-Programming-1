...
namespace Pathfinding
{
    ...
    public class GridGraph : NavGraph, IUpdatableGraph, ITransformedGraph
    {
        ...
        public static readonly float StandardIsometricAngle =
            90 - Mathf.Atan(1 / Mathf.Sqrt(2)) * Mathf.Rad2Deg;
        ...
        )
        {
            ...
            GetNodes(node =>
            {
                ...
                node.position = GraphPointToWorld(
                    gnode.XCoordinateInGrid,
                    gnode.ZCoordinateInGrid,
                    height
                );
            });
        }
        ...
        public static float ConvertHexagonSizeToNodeSize(
            InspectorGridHexagonNodeSize mode,
            float value
        )
        {
            if (mode == InspectorGridHexagonNodeSize.Diameter)
                value *= 1.5f / (float)System.Math.Sqrt(2.0f);
            else if (mode == InspectorGridHexagonNodeSize.Width)
                value *= (float)System.Math.Sqrt(3.0f / 2.0f);
            return value;
        }

        public static float ConvertNodeSizeToHexagonSize(
            InspectorGridHexagonNodeSize mode,
            float value
        )
        {
            if (mode == InspectorGridHexagonNodeSize.Diameter)
                value *= (float)System.Math.Sqrt(2.0f) / 1.5f;
            else if (mode == InspectorGridHexagonNodeSize.Width)
                value *= (float)System.Math.Sqrt(2.0f / 3.0f);
            return value;
        }
        ...
        private GridNode GetNodeConnection(int index, int x, int z, int dir)
        {
            ...
            if (nx < 0 || nx >= Width)
                return null;
            /// <summary>TODO: Modify to get adjacent grid graph here</summary>
        ...
        public GraphTransform CalculateTransform()
        {
            ...
            var isometricMatrix = Matrix4x4.TRS(
                Vector3.zero,
                Quaternion.Euler(0, 45, 0),
                Vector3.one
            );
            isometricMatrix =
                Matrix4x4.Scale(new Vector3(Mathf.Cos(Mathf.Deg2Rad * isometricAngle), 1, 1))
                * isometricMatrix;
            isometricMatrix =
                Matrix4x4.TRS(Vector3.zero, Quaternion.Euler(0, -45, 0), Vector3.one)
                * isometricMatrix;
            ...
            var boundsMatrix =
                Matrix4x4.TRS(center, Quaternion.Euler(rotation), new Vector3(aspectRatio, 1, 1))
                * isometricMatrix;
            ...
            var m =
                Matrix4x4.TRS(
                    ...
                    new Vector3(newNodeSize * aspectRatio, 1, newNodeSize)
                ) * isometricMatrix;
        ...
        )
        {
            ...
            nn.clampedPosition = transform.Transform(
                new Vector3(Mathf.Clamp(xf, x, x + 1f), y, Mathf.Clamp(zf, z, z + 1f))
            );
        ...
        public override NNInfoInternal GetNearestForce(Vector3 position, NNConstraint constraint)
        {
            ...
            if (constraint == null || constraint.Suitable(node))
            {
                ...
                clampedPosition = transform.Transform(
                    new Vector3(Mathf.Clamp(xf, x, x + 1f), y, Mathf.Clamp(zf, z, z + 1f))
                );
            }
            ...
            float maxDist =
                constraint == null || constraint.constrainDistance
            ...
            for (int w = 1; ; w++)
            {
                ...
                for (nx = x - w; nx <= x + w; nx++)
                {
                    ...
                    {
                        float dist = (
                            (Vector3)nodes[nx + nz2].position - globalPosition
                        ).sqrMagnitude;
                ...
                for (nz = z - w + 1; nz <= z + w - 1; nz++)
                {
                    ...
                    {
                        float dist = (
                            (Vector3)nodes[nx + nz * width].position - globalPosition
                        ).sqrMagnitude;
                        if (dist < minDist && dist < maxDistSqr)
                        {
                            minDist = dist;
        ...
        protected override IEnumerable<Progress> ScanInternal()
        {
            ...
            for (int z = 0; z < depth; z++)
            {
                ...
                if (progressCounter >= YieldEveryNNodes)
                {
                    ...
                    yield return new Progress(
                        Mathf.Lerp(0.1f, 0.7f, z / (float)depth),
            ...
            for (int z = 0; z < depth; z++)
            {
                ...
                if (progressCounter >= YieldEveryNNodes)
                {
                    progressCounter = 0;
                    yield return new Progress(
                        Mathf.Lerp(0.7f, 0.9f, z / (float)depth),
        ...
        public virtual void UpdateNodePositionCollision(
            GridNode node,
            int x,
            int z,
        ...
        public virtual void RecalculateCell(
            int x,
            int z,
            ...
            if (walkable && useRaycastNormal && collision.heightCheck)
            {
                if (hit.normal != Vector3.zero)
                {
                    ...
                    if (penaltyAngle && resetPenalties)
                    {
                        node.Penalty += (uint)
                            Mathf.RoundToInt(
                                (1F - Mathf.Pow(angle, penaltyAnglePower)) * penaltyAngleFactor
                            );
                    }
        ...
        void ErodeNodeWithTags(GraphNode node, int iteration)
        {
            ...
            if (
                ...
                && gnode.Tag < erosionFirstTag + iteration
            )
            {
                if (neighbours == NumNeighbours.Six)
                {
        ...
        public virtual bool IsValidConnection(GridNodeBase node1, GridNodeBase node2)
        {
            ...
            if (transform.onlyTranslational)
            {
                ...
                return System.Math.Abs(node1.position.y - node2.position.y)
                    <= maxClimb * Int3.Precision;
            }
            else
            {
                    ...
                    if (cutCorners)
                    {
                        for (int i = 0; i < 4; i++)
                        {
                            ...
                            if (((conns >> i | conns >> (i + 1) | conns >> (i + 1 - 4)) & 1) != 0)
                            {
                                ...
                                if (nx >= 0 & nz >= 0 & nx < width & nz < depth)
                                {
                                    GridNode other = nodes[
                                        index + neighbourOffsets[directionIndex]
                                    ];
        ...
        public override void OnDrawGizmos(RetainedGizmos gizmos, bool drawNodes)
        {
                ...
                if (drawNodes && width * depth * LayerCount != nodeCount)
                {
                    ...
                    for (int z = 0; z < d; z++)
                    {
                        helper.builder.DrawLine(
                            trans.Transform(new Vector3(0, 0, z)),
                            trans.Transform(new Vector3(w, 0, z)),
                            color
                        );
                    }
                    ...
                    {
                        helper.builder.DrawLine(
                            trans.Transform(new Vector3(x, 0, 0)),
                            trans.Transform(new Vector3(x, 0, d)),
                            color
                        );
                    }
                }
            ...
            GridNodeBase[] allNodes = ArrayPool<GridNodeBase>.Claim(
                chunkWidth * chunkWidth * LayerCount
            );
            for (int cx = width / chunkWidth; cx >= 0; cx--)
            {
                for (int cz = depth / chunkWidth; cz >= 0; cz--)
                {
                    ...
                    var allNodesCount = GetNodesInRegion(
                        new IntRect(
                            cx * chunkWidth,
                            cz * chunkWidth,
                            (cx + 1) * chunkWidth - 1,
        ...
        )
        {
            ...
            var neighbourIndices =
                neighbours == NumNeighbours.Six ? hexagonNeighbourIndices : new[] { 0, 1, 2, 3 };
            ...
            for (int i = 0; i < nodeCount; i++)
            {
                ...
                for (int dIndex = 0; dIndex < neighbourIndices.Length; dIndex++)
                {
                    ...
                    if (n1 != null)
                        p.y += transform.InverseTransform((Vector3)n1.position).y;
                    if (n2 != null)
                        p.y += transform.InverseTransform((Vector3)n2.position).y;
                    if (n3 != null)
                        p.y += transform.InverseTransform((Vector3)n3.position).y;
                    p.y /= (
                        1f
                        + (n1 != null ? 1f : 0f)
                        + (n2 != null ? 1f : 0f)
                ...
                {
                    var other = node.GetNeighbourAlongDirection(
                        neighbourIndices[(j + 1) % neighbourIndices.Length]
                    );
                    ...
                    if (
                        other == null
                        || (showMeshOutline && node.NodeInGridIndex < other.NodeInGridIndex)
                    )
                    {
                        helper.builder.DrawLine(
                            vertices[baseIndex + j],
            ...
            if (showMeshSurface)
                helper.DrawTriangles(
                    vertices,
                    colors,
                    baseIndex * trianglesPerNode / verticesPerNode
                );
        ...
        protected virtual List<GraphNode> GetNodesInRegion(Bounds bounds, GraphUpdateShape shape)
        {
            ...
            for (int x = rect.xmin; x <= rect.xmax; x++)
            {
                for (int z = rect.ymin; z <= rect.ymax; z++)
                {
                    ...
                    if (
                        bounds.Contains((Vector3)node.position)
        ...
        protected void CalculateAffectedRegions(
            GraphUpdateObject o,
