...
namespace Pathfinding
{
    ...
    public class GridNode : GridNodeBase
    {
        ...
        public void SetConnectionInternal(int dir, bool value)
        {
            ...
            {
                gridFlags = (ushort)(
                    gridFlags & ~((ushort)1 << GridFlagsConnectionOffset << dir)
        ...
        public void SetAllConnectionInternal(int connections)
        {
            ...
            {
                gridFlags = (ushort)(
                    (gridFlags & ~GridFlagsConnectionMask)
        ...
        public bool EdgeNode
        {
            ...
            set
            {
                ...
                {
                    gridFlags = (ushort)(
                        gridFlags & ~GridFlagsEdgeNodeMask | (value ? GridFlagsEdgeNodeMask : 0)
                    );
                }
            }
        }
        ...
        public override Vector3 ClosestPointOnNode(Vector3 p)
        {
            ...
            var closestInGraphSpace = new Vector3(
                Mathf.Clamp(p.x, x, x + 1f),
                y,
                Mathf.Clamp(p.z, z, z + 1f)
            );
        ...
        )
        {
            ...
            {
                if (
                    HasConnectionInDirection(i)
                    && other == nodes[NodeInGridIndex + neighbourOffsets[i]]
                )
                {
                    Vector3 middle = ((Vector3)(position + other.position)) * 0.5f;
                    Vector3 cross = Vector3.Cross(
                        gg.collision.up,
                        (Vector3)(other.position - position)
                    );
                    cross.Normalize();
