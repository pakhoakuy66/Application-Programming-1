    ...
    public class Funnel
    {
        ...
        public static List<PathPart> SplitIntoParts(Path path)
        {
            ...
            for (int i = 0; i < nodes.Count; i++)
            {
                if (nodes[i] is TriangleMeshNode || nodes[i] is GridNodeBase)
                {
                    ...
                    {
                        if (
                            nodes[i].GraphIndex != currentGraphIndex && !(nodes[i] is NodeLink3Node)
                        )
                        {
                            break;
                        }
                ...
                else if (NodeLink2.GetNodeLink(nodes[i]) != null)
                {
                    ...
                    else if (i - part.startIndex != 1)
                    {
                        throw new System.Exception(
                            "NodeLink2 link length greater than two (2) nodes. "
                                + (i - part.startIndex + 1)
                        );
                    }
        ...
        public static void Unwrap(FunnelPortals funnel, Vector2[] left, Vector2[] right)
        {
            ...
            var normal = Vector3.Cross(
                funnel.right[1] - funnel.left[0],
                funnel.left[1] - funnel.left[0]
            );
            ...
            while (normal.sqrMagnitude <= 0.00000001f && startingIndex + 1 < funnel.left.Count)
            {
                ...
                normal = Vector3.Cross(
                    funnel.right[startingIndex] - funnel.left[0],
        ...
        static int FixFunnel(Vector2[] left, Vector2[] right, int numPortals)
        {
            if (numPortals > left.Length || numPortals > right.Length)
                throw new System.ArgumentException(
                    "Arrays do not have as many elements as specified"
                );
            ...
            int startIndex = 0;
            while (
                left[startIndex + 1] == left[startIndex + 2]
                && right[startIndex + 1] == right[startIndex + 2]
            )
            {
        ...
        public static List<Vector3> Calculate(
            FunnelPortals funnel,
            bool unwrap,
        ...
        )
        {
            ...
            for (int i = 0; i < intermediateResult.Count; i++)
            {
                ...
                if (splitAtEveryPortal)
                {
                    ...
                    {
                        var factor = VectorMath.LineIntersectionFactorXZ(
                            FromXZ(leftArr[j]),
                            FromXZ(rightArr[j]),
                            FromXZ(prev2D),
                            FromXZ(next2D)
        ...
        )
        {
            ...
            for (int i = startIndex + 2; i < numPortals; i++)
            {
                ...
                {
                    Debug.LogWarning(
                        "Avoiding infinite loop. Remove this check if you have this long paths."
                    );
                    break;
                }
                ...
                if (LeftOrColinear(portalRight - portalApex, pRight - portalApex))
                {
                    if (
                        ...
                        || RightOrColinear(portalLeft - portalApex, pRight - portalApex)
                    )
                    {
                        portalRight = pRight;
                        rightIndex = i;
                ...
                if (RightOrColinear(portalLeft - portalApex, pLeft - portalApex))
                {
                    if (
                        ...
                        || LeftOrColinear(portalRight - portalApex, pLeft - portalApex)
                    )
                    {
                        portalLeft = pLeft;
                        leftIndex = i;
