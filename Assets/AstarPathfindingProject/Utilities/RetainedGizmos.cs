    ...
    public class RetainedGizmos
    {
        ...
        public struct Hasher
        {
            ...
            public Hasher(AstarPath active)
            {
                ...
                includePathSearchInfo =
                    debugData != null
                    && (
                        active.debugMode == GraphDebugMode.F
        ...
        public class Builder : IAstarPooledObject
        {
            ...
            public void DrawWireCube(GraphTransform tr, Bounds bounds, Color color)
            {
                ...
                DrawLine(
                    tr.Transform(new Vector3(min.x, min.y, min.z)),
                    tr.Transform(new Vector3(max.x, min.y, min.z)),
                    color
                );
                DrawLine(
                    tr.Transform(new Vector3(max.x, min.y, min.z)),
                    tr.Transform(new Vector3(max.x, min.y, max.z)),
                    color
                );
                DrawLine(
                    tr.Transform(new Vector3(max.x, min.y, max.z)),
                    tr.Transform(new Vector3(min.x, min.y, max.z)),
                    color
                );
                DrawLine(
                    tr.Transform(new Vector3(min.x, min.y, max.z)),
                ...
                DrawLine(
                    tr.Transform(new Vector3(min.x, max.y, min.z)),
                    tr.Transform(new Vector3(max.x, max.y, min.z)),
                    color
                );
                DrawLine(
                    tr.Transform(new Vector3(max.x, max.y, min.z)),
                    tr.Transform(new Vector3(max.x, max.y, max.z)),
                    color
                );
                DrawLine(
                    tr.Transform(new Vector3(max.x, max.y, max.z)),
                    tr.Transform(new Vector3(min.x, max.y, max.z)),
                    color
                );
                DrawLine(
                    tr.Transform(new Vector3(min.x, max.y, max.z)),
            ...
            void SubmitLines(RetainedGizmos gizmos, ulong hash)
            {
                ...
                int batches =
                    (lines.Count + MaxLineEndPointsPerBatch - 1) / MaxLineEndPointsPerBatch;
        ...
        Mesh GetMesh()
        {
            ...
            {
                return new Mesh { hideFlags = HideFlags.DontSave };
            }
        }
        ...
        public void FinalizeDraw()
        {
            ...
            if (surfaceMaterial == null)
                surfaceMaterial =
                    UnityEditor.AssetDatabase.LoadAssetAtPath(
                        EditorResourceHelper.editorAssets + "/Materials/Navmesh.mat",
                        typeof(Material)
                    ) as Material;
            ...
            for (int matIndex = 0; matIndex <= 1; matIndex++)
            {
                ...
                for (int pass = 0; pass < mat.passCount; pass++)
                {
                    ...
                    {
                        if (
                            meshes[i].lines == (mat == lineMaterial)
