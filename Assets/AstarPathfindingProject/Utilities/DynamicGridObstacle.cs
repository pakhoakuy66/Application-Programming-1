    ...
    public class DynamicGridObstacle : GraphModifier
    {
        ...
        protected override void Awake()
        {
            ...
            if (coll == null && coll2D == null && Application.isPlaying)
            {
                throw new System.Exception(
                    "A collider or 2D collider must be attached to the GameObject("
                        + gameObject.name
        ...
        void Update()
        {
            ...
            while (
                ...
                && pendingGraphUpdates.Peek().stage != GraphUpdateStage.Pending
            )
            {
            ...
            if (colliderEnabled)
            {
                ...
                var errorFromRotation =
                    extents * Quaternion.Angle(prevRotation, newRotation) * Mathf.Deg2Rad;

                // If the difference between the previous bounds and the new bounds is greater than some value, update the graphs
                if (
                    minDiff.sqrMagnitude > updateError * updateError
                    || maxDiff.sqrMagnitude > updateError * updateError
                    || errorFromRotation > updateError
                    || !prevEnabled
                )
                {
