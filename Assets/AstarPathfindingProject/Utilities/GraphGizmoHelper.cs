...
namespace Pathfinding.Util
{
    public class GraphGizmoHelper : IAstarPooledObject, System.IDisposable
    {
                ...
                if (InSearchTree(node, debugData, debugPathID))
                {
                    ...
                    if (pnode.parent != null)
                    {
                        builder.DrawLine(
                            ...
                            (Vector3)debugData.GetPathNode(node).parent.node.position,
                            NodeColor(node)
                        );
                    }
                }
        ...
        void DrawConnection(GraphNode other)
        {
            builder.DrawLine(
                drawConnectionStart,
                Vector3.Lerp((Vector3)other.position, drawConnectionStart, 0.5f),
                drawConnectionColor
            );
        }
            ...
            if (node.Walkable)
            {
                switch (debugMode)
                {
                    ...
                    case GraphDebugMode.Penalty:
                        color = Color.Lerp(
                            AstarColor.ConnectionLowLerp,
                            AstarColor.ConnectionHighLerp,
