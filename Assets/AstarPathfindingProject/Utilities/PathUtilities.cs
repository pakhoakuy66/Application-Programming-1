...
namespace Pathfinding
{
    ...
    public static class PathUtilities
    {
        ...
        public static List<GraphNode> GetReachableNodes(
            GraphNode seed,
            int tagMask = -1,
        ...
        ///      Only walkable nodes are searched regardless of the filter. If the filter function returns false the node will be treated as unwalkable.</param>
        public static List<GraphNode> BFS(
            GraphNode seed,
            int depth,
            int tagMask = -1,
            System.Func<GraphNode, bool> filter = null
        ...
        {
            return new Vector3(
                a * (Mathf.Cos(t) + t * Mathf.Sin(t)),
                0,
                a * (Mathf.Sin(t) - t * Mathf.Cos(t))
            );
        }
        ...
        )
        {
            ...
            radius = Mathf.Max(
                radius,
                1.4142f * clearanceRadius * Mathf.Sqrt(previousPoints.Count)
            ); //Mathf.Sqrt(previousPoints.Count*clearanceRadius*2));
