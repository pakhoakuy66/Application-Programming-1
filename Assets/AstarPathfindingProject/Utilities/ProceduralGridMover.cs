...
namespace Pathfinding
{
    ...
    public class ProceduralGridMover : VersionedMonoBehaviour
    {
                ...
                if (graphIndex >= AstarPath.active.data.graphs.Length)
                    throw new System.Exception(
                        "The ProceduralGridMover was configured to use graph index "
                            + graphIndex
                            + ", but only "
                            + AstarPath.active.data.graphs.Length
                            + " graphs exist"
                    );
        ...
        void Update()
        {
            ...
            if (
                VectorMath.SqrDistanceXZ(graphCenterInGraphSpace, targetPositionInGraphSpace)
        ...
        IEnumerator UpdateGraphCoroutine()
        {
            ...
            if (Mathf.Abs(offset.x) <= width && Mathf.Abs(offset.y) <= depth)
            {
                ...
                connectionRect = IntRect.Intersection(
                    connectionRect,
                    new IntRect(0, 0, width, depth)
                );
                ...
                for (int l = 0; l < layers; l++)
                {
                    ...
                    for (int z = 0; z < depth; z++)
                    {
                        ...
                        {
                            buffer[tz + ((x + offset.x + width) % width)] = nodes[
                                layerOffset + pz + x
                            ];
                        }
                    }
